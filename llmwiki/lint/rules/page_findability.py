"""page_findability — titled pages must be findable by their own title (#197)."""
...
class PageFindability(LintRule):
    """Error when a titled page is not returned for its own title."""

    name = "page_findability"
    severity = "error"
    description = (
        "A titled page is not returned when searched by its own title "
        "(score ≤ 0, absent from the search corpus, or cut short by the "
        "result cap)"
    )
