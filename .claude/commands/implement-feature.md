...
**Author field (#159 / §10):** After `/awos:spec` and `/awos:tech` write (or amend) their documents, ensure **Author** / **Author(s)** is the operator’s human name — never command names, issue ids, “AWOS”, “Auto”, or other agent/tool metadata. Resolve per run: session/host display name when available; else `git config user.name` when it looks like a human name (prefer a multi-word display name over a bare nickname); if only a nickname is available, ask once for the display name for this run. Ticket linkage stays on **Roadmap Item** / the issue line elsewhere in the header.
