...
**Author field (#159 / §10):** After `/awos:spec` and `/awos:tech` write (or amend) their documents, ensure **Author** / **Author(s)** is exactly `Alexander Makarov` — human maintainer name only. Do not leave command names, issue ids, “AWOS”, “Auto”, or usernames in that field. Ticket linkage stays on **Roadmap Item** / the issue line elsewhere in the header.
