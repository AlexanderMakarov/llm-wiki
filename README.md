...
- **A static site for humans** — Home, Analytics, Candidates, Topics, graph, and `Cmd+K` search split into Wiki and Site results. See what the pipeline did, browse settled entities/concepts, and search wiki text with the same literal-match rules agents use. Generated by `llmwiki build`; see the [UI reference](docs/reference/ui.md).
