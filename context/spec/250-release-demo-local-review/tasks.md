...
- [x] **Slice 1: `release_demo_gate.py` (usage + build + URL + case-fold)**
  ...
  - [x] Add `tests/test_release_demo_gate.py`: `--dry-run` exits 0 and prints planned URL/steps; failure path returns non-zero (mock/subprocess); `--today` forwarded to usage path. Keep tests off the live vault and avoid rewriting committed `demo/usage/` on the default happy path (use tmp / dry-run / mocks). **[Agent: testing-expert]**
  - [x] Verify: `ruff check scripts/release_demo_gate.py tests/test_release_demo_gate.py`; `python3 -m pytest tests/test_release_demo_gate.py -q`. **[Agent: testing-expert]**

- [x] **Slice 2: Thin skill + RELEASE_PROCESS around the gate**
  ...
  - [x] Update `docs/maintainers/RELEASE_PROCESS.md`: after sessions/docs/synth completeness, run `release_demo_gate.py --today …`; commit `demo/usage/` (+ keep `#255` state bullet); human reviews printed URL before tag push; clarify Pages/CI only build + version-assert. Keep incomplete-synth hard stop. **[Agent: generalPurpose]**
  - [x] Update `.claude/skills/release/SKILL.md` to thin wrappers around the script + human pause; fix “filenames change” → in-place-by-slug alignment with process doc. Optional one-liner in `docs/maintainers/REFRESH_DEMO.md`. CHANGELOG Unreleased Changed bullet for #240. **[Agent: generalPurpose]**
  - [x] Extend `tests/test_209_release_skill.py`: skill + RELEASE_PROCESS mention `release_demo_gate`, human pause after gate / local URL, and CI does not invent sessions/usage. Run those tests + `ruff` on touched Python. **[Agent: testing-expert]**

- [x] **Slice 3: Feature Testing & Regression**

  > Verifies FR1–FR6 against functional-spec.md after slices 1–2.

  - [x] Map FR1–FR6 to tests/docs evidence; fill any gap with `@spec: 250-release-demo-local-review` coverage. Confirm `pages.yml` still has no session/usage generation. **[Agent: testing-expert]**
  - [x] Full `python3 -m pytest tests/ -q` and `ruff check llmwiki tests scripts` on the worktree; fix failures. **[Agent: testing-expert]**
