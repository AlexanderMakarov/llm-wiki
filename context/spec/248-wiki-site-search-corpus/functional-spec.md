...
- **As a reader, I want the site's search to find wiki knowledge exactly as my assistant does, shown separately from ordinary site pages**, so that the site and my assistant never answer the same question differently, and so that I can tell curated knowledge from site navigation at a glance.
  - **Acceptance Criteria:**
    ...
    - [x] Given a matching term falls late in a long line, when I read the row, then the displayed excerpt starts near the hit rather than clipping the highlight off the right edge.
    - [x] Given a wiki summary and a raw session or document both match and open the same reader page, when results appear, then the Wiki row is shown once and the destination is not repeated in the Site group.
...
- **Automated:** `tests/test_248_palette_match.py` has 30 passing node-driven tests, including the operator-smoke regressions while retaining equality with `llmwiki.search.engine.search_match` over the real 205-page demo corpus; the safe full suite is 5415 tests, 0 failures, 48 skipped, and `ruff` is clean. Three unrelated cases from the 5418-test collection were deselected because this sandbox blocks a real-home automation write and network downloads for two isolated wheel builds; the preceding stable-tree run before the two smoke fixes was 5416 tests, 0 failures, 48 skipped. Whole-feature acceptance remains in `tests/test_248_acceptance.py` (9 tests, RED-validated against a detached worktree at `179d00c`).
...
- **`file://` operator smoke:** the palette loaded and searched both payloads directly from disk, closing the original mechanism-only limitation. That smoke exposed a clipped late-in-line mark and a shared Wiki/Site destination; both now have node-driven regressions and a rebuilt plain-file preview awaiting the operator's visual re-check.
...
One limitation remains, stated rather than papered over:
