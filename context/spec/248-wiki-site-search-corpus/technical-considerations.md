...
**Presentation.** Two groups, WIKI first, **both always rendered** — a group with no hits shows its heading and a zero-results line rather than disappearing, so the reader can tell "nothing in the wiki" from "search is broken". Caps are MCP's: `DEFAULT_PAGE_CAP = 200` and `DEFAULT_HIT_CAP = 200` (`search/engine.py:23-24`); a group that hits a cap says so, mirroring MCP's `truncated` flag.

**Index shape.** Substring matching over page bodies, and returning matching *lines*, both require the page text client-side, so the full-text decision stands (`DEFAULT_PER_FILE_CAP` is 4 MiB, effectively uncapped). Lines are split in the browser rather than precomputed, keeping the payload plain text.
