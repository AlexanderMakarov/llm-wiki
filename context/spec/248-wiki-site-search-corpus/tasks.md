...
- [x] **Slice 5: The build emits the wiki corpus as a lazy search payload**
  ...
  - [x] In `llmwiki/build.py`, emit the wiki corpus as its own payload by reusing the deterministic, containment-checked `iter_scanned_pages` reader: **every readable `.md` under `wiki/` except `archive/`**, `_context.md` **included**, with the shared 4 MiB per-file and 50 MiB aggregate caps. Carry complete retained page text plus `rel_path`, `title`, and the reader URL from `_compute_site_url` (`graph.py:94-140`), which is `None` for `overview.md`, `log.md`, `candidates/`, `syntheses/` and `categories/`. Write it with the `.json` + `.js` sidecar treatment via `write_js_sidecar` (#20), reference it from an optional manifest key beside `_chunks`, and publish cap-completeness status so the viewer can warn. **[Agent: general-purpose]**
