...
- [ ] **Slice 7: Documentation and changelog**
  ...
  - [ ] `.github/workflows/e2e.yml`: add `llmwiki/render/**` to BOTH the `push` and `pull_request` path filters. The palette lives in `llmwiki/render/js.py`, which is absent from the current list — this PR triggers e2e only incidentally via `llmwiki/build.py`, so a future palette-only change would skip browser tests entirely. Two lines; do not restructure the workflow. **[Agent: general-purpose]**
