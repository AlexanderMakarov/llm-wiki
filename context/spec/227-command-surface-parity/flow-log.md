...
The `diagnose` finding #1 above is **wrong** and is kept only as a record of what was believed at the time.

Corrected discovery model, established empirically with `cursor-agent` stream-json probes: Cursor **does** load slash commands from top-level `.claude/commands/*.md`. A throwaway command placed only in that directory was answered with **zero** tool calls — client-side command expansion, so the harness had already resolved the file. The byte-identical file placed under an ordinary non-command directory instead took **14** tool calls, with the model narrating that it was searching for whatever the slash referred to. Cursor also reads `.claude/skills/` and `.claude/agents/` natively (which the `verify-criteria` note already recorded).

What Cursor does **not** load is a nested namespace, `.claude/commands/<ns>/<name>.md` — confirmed by the maintainer. That is the only real gap, and it is why the flat `.cursor/commands/awos-*.md` wrappers generated by `scripts/sync-awos-cursor-commands.sh` stay necessary.

Consequence: `/fix-bug` and `/implement-feature` were always invocable from Cursor, so AC #1 of the issue rested on a false premise. `scripts/sync-cursor-commands.sh` and the two `.cursor/commands/` wrappers it generated were removed from this branch, along with its call in `scripts/update-awos.sh`. `tests/test_command_surface_parity.py` was reworked around the invariants that hold: nested Claude namespaces must have a flat Cursor wrapper, and a top-level Claude command must not acquire a redundant Cursor duplicate (`release` allowlisted with a reason). `docs/maintainers/{README,AWOS-CURSOR}.md` and `CHANGELOG.md` were corrected to the model above; the `/triage-issue` retirement note in `DECLINED.md` (and its echoes in `TRIAGE.md` / `CHANGELOG.md`) now says `/fix-bug` is the replacement entry point.
