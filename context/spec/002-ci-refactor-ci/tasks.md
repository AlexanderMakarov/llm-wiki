...
- [ ] **Slice 1: Remove dead Claude-in-CI workflows**
  - [ ] Delete `.github/workflows/claude.yml` and `.github/workflows/claude-code-review.yml`. **[Agent: generalPurpose]**
  - [ ] Verify: `rg -n 'claude-code-action|CLAUDE_CODE_OAUTH_TOKEN' .github/` returns no matches. **[Agent: generalPurpose]**

- [ ] **Slice 2: Bump GitHub Actions to Node-24 majors**
  - [ ] Update every in-scope action pin per `technical-considerations.md` (checkout@v7, setup-python@v7, setup-node@v7, upload-artifact@v7, download-artifact@v8, configure-pages@v6, create-pull-request@v8, gitleaks-action@v3). Keep upload/download paired in `release.yml` and elsewhere. Do not bump out-of-scope actions listed in the tech spec. **[Agent: generalPurpose]**
