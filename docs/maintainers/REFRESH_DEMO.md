...
- It does **not** refresh authored demo **sessions**. On a release cut the default is to run `scripts/generate_demo_sessions.py` with a release-day `--today` (and re-synth when filenames change) unless the human explicitly opts out — see [RELEASE_PROCESS.md](RELEASE_PROCESS.md) and #225.
