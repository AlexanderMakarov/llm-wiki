...
| `synthesis` | `claude_model` | string | `"sonnet"` | Legacy flat model alias for the `claude` backend (prefer `synthesis.claude.model`) |
| `synthesis` | `claude_path` | string | `""` | Legacy flat path to the `claude` binary (prefer `synthesis.claude.path`) |
| `synthesis` | `claude_timeout` | int (s) | 180 | Legacy flat per-page timeout for `claude` (prefer `synthesis.claude.timeout`). Separate from `synthesis.ollama.timeout` — before v1.4.1 both backends shared one `timeout` key, so the Ollama default silently capped claude pages at 60s |
| `synthesis` | `claude_effort` | enum | unset | Legacy flat `--effort` for `claude` (prefer `synthesis.claude.effort`: `low`/`medium`/`high`/`xhigh`/`max`). Extended thinking is billed as output at ~5x input; on Haiku it was 5,753 output tokens/page at the default vs 1,609 at `low`. Set `low` on small models |
...
| `synthesis` | `claude_lean` | bool | true | Legacy flat lean flag for `claude` (prefer `synthesis.claude.lean`). Strip agent scaffolding (tool schemas, MCP servers, skills, `CLAUDE.md`, agent system prompt) from each `claude` call — ~9x cheaper per page, measured. Only an explicit `false` opts out. See [reference/synthesis-cost.md](reference/synthesis-cost.md) |
| `synthesis.claude` | `model` | string | `"sonnet"` | Nested Claude model id / alias. Nested wins over flat `claude_model`. |
| `synthesis.claude` | `path` | string | `""` | Nested path to the `claude` binary (overrides `$PATH`). Nested wins over flat `claude_path`. |
| `synthesis.claude` | `timeout` | int (s) | 180 | Nested per-page timeout. Nested wins over flat `claude_timeout`. |
| `synthesis.claude` | `lean` | bool | true | Nested lean mode. Nested wins over flat `claude_lean`. |
| `synthesis.claude` | `effort` | enum | unset | Nested `--effort`. Nested wins over flat `claude_effort`. |
