...
| Path | Role |
|---|---|
| `usage/mcp-<pid>-<start>.jsonl` | Live, append-only MCP logs — one file per MCP server process, so concurrent editors never contend on a shared write lock. |
| `usage/rollup.json` | Lifetime MCP totals after monthly compact — the kept-forever aggregate once raw JSONL for past months is folded and deleted. |
| `usage/daily.json` | Per-day MCP counts that survive compact — `mcp_calls`, `retrievals`, `writes`, `session_reads`, `doc_reads`, `by_tool`, attribution counters, and related fields. |
| `llmwiki-state.json` | Synth queue, sync mtimes, cost estimate, quarantine — separate from `usage/`; MCP telemetry never writes here. |
| `raw/sessions/*.md` frontmatter | Session-side signal — `tools_used`, `tool_counts`, dates — used for wiki-adoption heatmaps and best-effort session/day counts alongside MCP logs. |
