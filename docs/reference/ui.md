...
Numbers come from `llmwiki-state.js` (`synth.pipeline` + `synth.pending` + `synth.estimate`), refreshed by `llmwiki sync` / `llmwiki synthesize --estimate`. `llmwiki build` also one-shot backfills `synth.pipeline` when the state snapshot predates that key (v1.4→v1.5 upgrade — #70); later builds skip the refresh once the shape is present. The session-analytics content (heatmap, stats, project grid) lives on [Analytics](#analytics).
