...
**The site's quick search mirrors this mode.** The `⌘K` command palette's **Wiki** result group runs the same algorithm over the same corpus — every `.md` under `wiki/` except `archive/` — with the same rules: literal case-insensitive substring, name matches (title or path) before body-only matches, each sorted by path, the same `kind` filter, and the same 200-page / 200-matching-line caps. A term typed on the site and the same term given to `wiki_search` return the same pages in the same order.
