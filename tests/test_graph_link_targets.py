...
These assert on the emitted ``graph-viewer.js`` sibling of ``graph.html`` so a
...
def test_panel_open_page_link_stays_in_the_current_tab(viewer_js: str):
...
def test_panel_session_links_stay_in_the_current_tab(viewer_js: str):
    sessions = _js_block(viewer_js, "function topicSessionLinks(slugs, limit) {")
    assert "panel-link" in sessions
    assert "_blank" not in sessions


def test_context_menu_open_navigates_in_the_current_tab(viewer_js: str):
    menu = _js_block(viewer_js, "if (ctxMenu) ctxMenu.addEventListener('click'")
...
def test_double_click_still_opens_a_new_tab(viewer_js: str):
    """The one deliberate exception — double-click means "take this elsewhere"."""
    handler = _js_block(viewer_js, "network.on('doubleClick', params => {")
