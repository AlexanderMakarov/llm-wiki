...
name: repo-auditor
description: >-
  Audit ONE repository end-to-end for the AI-readiness audit — the deterministic
  engine pass plus the LLM-only slice — writing all results into a
  caller-provided per-repo output subdir. The ai-readiness-audit org-mode flow
  dispatches one of these per repo, concurrently, so a portfolio audits in
