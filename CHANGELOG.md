...
- **MCP usage + raw-docs on the Analytics pages (#27)** — the static site now answers "is this wiki earning its keep, and who actually uses it?" without leaving the Analytics page. A new **Wiki usage (MCP)** section shows lifetime MCP totals baked in at `build` time: per-tool **calls** and **items returned** (the latter counted only for entity-returning tools — `query`/`search`/`list_sources`/`read_page`/`entity_search`/`category_browse`/`export`/`confidence` — so `lint`/`sync`/`lifecycle`/`dashboard` don't inflate it), each tool's **zero-hit rate**, a **raw documents** total, and a **Most MCP-active project** card. Response bytes are collected but no longer surfaced — a low-signal metric. The distinct-client metric is reported honestly as **MCP server processes** (distinct `(server_pid, server_started)` pairs), never as "sessions". Per-**project** pages gain a matching stats block: raw-doc count, MCP calls, items returned, and server processes for that project. The activity heatmap is renamed **Agents Activity** (it covers agent sessions, not raw docs), the "Total tokens" + "Average per session" cards merge into one **Tokens** card, "Heaviest project" is clarified to **Heaviest project (by tokens)**, and **Recent activity** renders a producer breakdown (e.g. `2 Claude · 1 Cursor · 0 docs`) verbatim when the log entry provides one. Aggregation reuses `usage.combined_totals`, so numbers are as-of-last-build; a legacy `rollup.json` lacking the new fields reads as zero.
