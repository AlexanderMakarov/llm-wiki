...
- **`llmwiki migrate-topic-kinds --vault PATH` (#174)** — offline one-time stamp of missing `(entity|concept)` labels onto older source Connections bullets from pages already under `wiki/entities/`, `wiki/concepts/`, and matching `wiki/candidates/` folders (no synthesis backend or network call). Complements the #147 catch-up: clears the topics-rewrite flag when at least one resolvable kind can be stamped, without inventing `fact:` lines. `--dry-run` previews; a successful apply writes vault-root `.llmwiki-topic-kinds-stamped.json` so a later `synth --force --path …` can target exactly those `source_file` entries. Docs: `docs/UPGRADING.md` (beside #147), `docs/reference/cli.md`.
  - *Release note:* Offline `migrate-topic-kinds` stamps missing topic kinds from the wiki on disk (#174).
