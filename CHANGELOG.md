...
- **`install-automation` pins the scheduled working directory to the git main worktree (#206)** — the generated wrapper script `cd`s into a working directory before running `python3 -m llmwiki`, and that directory's `config.json` supplies the scheduled run's `filters.since` / adapter lookback. Running `install-automation` from a linked git worktree (whose own `config.json` is typically empty) previously baked that worktree in as the working directory, so a scheduled run could silently ignore the primary checkout's lookback and adapter settings — notably converting far more Cursor IDE history than expected. `resolve_main_worktree()` now resolves the git main worktree (via `git worktree list --porcelain`) and uses it as `working_dir` whenever install is run from a linked worktree; a one-line notice is printed when the two differ. Non-git installs (e.g. an installed package) are unaffected; if detection fails, the previous directory is used with a stderr warning.
  - *Release note:* `install-automation` now schedules jobs against the git main worktree's `config.json`, so lookback/adapter settings match your primary checkout even when you ran the installer from a linked worktree (#206).
