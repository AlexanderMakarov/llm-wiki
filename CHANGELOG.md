...
- **Sessions table Date column shows the full `YYYY-MM-DD`** — the Date `<col>` was 10% under `table-layout: fixed` with ellipsis on every cell, so dates rendered as `2026-07-…`. Date is now 13% (taken from Session), table `min-width` is 920px, and the Date cell opts out of ellipsis.
