...
- **Candidates review UI (#97)** — dedicated `/candidates.html` with pending Entities / Concepts tables (Promote · Flip and promote · Discard · Merge-with dropdown of same-table peers). Home Knowledge-layer **Candidates** and Analytics **Candidates to review** link there. `llmwiki serve` hosts `POST /api/candidates` over the same library paths as `llmwiki candidates promote|flip-promote|merge|discard` (including Key Facts on promote from #103 and catalog reconcile from #101). `/wiki-candidates` documents all four actions; no hand-`mv` flip path.
