...
- **Sessions table sticky header no longer overlaps the first data row** — `.sessions-table thead` used `top: 56px` (nav height) under the assumption that sticky was viewport-relative, but `.table-wrap { overflow-x: auto }` is the sticky containing block, so that offset shoved the header down into the first `<tbody>` row on initial paint. Sticky `top` is now `0`.
