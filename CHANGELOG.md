...
- **The demo site never republished, so it served a four-release-old build (#213)** — `pages.yml` was `workflow_dispatch`-only (#69), which meant the live site only moved when someone remembered to press a button; it had been showing v1.5.0 since v2.1.0 shipped. It now also deploys on every `v*.*.*` tag, alongside `release.yml`. Deploy on push to `main` stays off — the demo tracks releases, not merges. The deploy job additionally reads `manifest.json` back off the live URL and fails when the served version isn't the `__version__` of the commit it just published, so a green run means the site really serves that release rather than merely that GitHub accepted the artifact. Both triggers assert against the checked-out `__version__`, so a manual dispatch is checked as strictly as a tag, and a tag pushed without the version bump fails instead of publishing a mislabelled site.
...
- **Nothing noticed when the published site fell behind the code (#213)** — new `scripts/check_live_version.py` (stdlib only) compares the `version` in a live site's `manifest.json` with an expected version, defaulting to `__version__` in the checked-out tree, with distinct exit codes for stale / unreachable / malformed so a failing job says which. A new weekly `pages-freshness.yml` workflow (Mondays 06:00 UTC, plus manual dispatch) runs it against the published demo and turns red when the live version has drifted, closing the gap the uptime checks left open: every URL returned 200 the whole time the demo was stale.
  - *Release note:* A weekly check flags a demo site that has drifted behind the released version; run it yourself with `python3 scripts/check_live_version.py --url <site-url>` (#213).
