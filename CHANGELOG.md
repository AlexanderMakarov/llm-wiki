...
- **`docs/roadmap.md` (the internal Phase × Layer × Item MoSCoW table)** — retired in favour of [`context/product/roadmap.md`](context/product/roadmap.md), the roadmap the AWOS loop actually reads. It had drifted into contradicting the product: it still listed the removed HTTP server and `serve.sh` / `serve.bat` as shipped Musts, described highlight.js as CDN-loaded after it was vendored, marked the L2 builder and L3 viewer as "current work", and carried a `Last updated: 2026-04-08` header. Scope decisions it held (Slack/Discord ingest, TUI browser, real-time collaborative editing, a precompiled Go/Rust binary, Sentry/telemetry, a Postgres backend, and the server-side FTS5 search fallback) moved to `docs/maintainers/DECLINED.md`; the five items with anything left to decide moved to the AWOS roadmap's *Later / deferred*. `docs/index.md`, `docs/architecture.md`, `docs/public-roadmap.md` and `docs/maintainers/ROADMAP.md` now point at the AWOS roadmap.
