...
- **Session pages surface `cwd` + real `sessionId` + a copyable resume command; project pages show disk path (#36)** — the static site was the browsing UI for session history but gave no way to get back into a session with the originating agent, and no way to tell where on disk a project lives. Session heroes now show `cwd` and the full agent `sessionId` (distinct from llmwiki's 8-hex slug), plus a copyable `cd <cwd> && claude --resume <sessionId>` one-liner for Claude Code main sessions (hidden for subagents and non-Claude adapters; greyed out when the session is older than Claude Code's ~30-day retention window). Per-page `.json` siblings and the HTML metadata comment gain `sessionId`; search-index chunk entries include it too so a session is findable by id. Project pages and `projects/index.html` show the project's disk path (most common `cwd` across its sessions; list all distinct cwds when they diverge). 17 tests in `tests/test_session_resume_cwd.py`.
