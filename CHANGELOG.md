...
- **Demo vault case-collision cleanup + CI guard (#160)** — removed `demo/wiki/candidates/entities/LLMWiki.md`, a stray case-duplicate of the dominant-spelling `demo/wiki/candidates/entities/llmwiki.md`; the two coexisted fine on case-sensitive Linux but collided on case-insensitive filesystems (macOS default, Windows). Added `tests/test_case_insensitive_paths.py` so the standard `pytest tests/` run fails when any git-tracked paths fold to the same lowercase form. The harvest writer's exact-spelling wikilink behavior is unchanged; that's tracked in #204.
  - *Release note:* No user-facing behavior change; the demo vault no longer ships a case-duplicate page, and CI now catches this class of collision before merge (#160).
