...
- **Analytics Wiki value section (#52)** — the Analytics page now leads with usage-led value signals, not just call volume. New **Wiki value** block: retrievals (query/search/read_page) · writes (`wiki_add`) · answer rate · payoff-per-page · distinct attributed caller projects (Tier 1; unattributed calls shown separately). Dual daily chart — MCP calls/day (agent-agnostic) plus best-effort wiki-using sessions/day (Claude `mcp__llmwiki__*` tools_used, Cursor `CallMcpTool` when the transcript names llmwiki). Sessions vs docs corpus mix and read mix (page counts, no USD split). Top-earning pages and dead stock from `wiki_read_page` paths joined to `wiki/sources` (retained/live telemetry only — not lifetime across compact). Daily MCP series persists in `usage/daily.json` (folded on compact, live overlay refreshed on every build). Synthesis USD stays a muted secondary line when `synth.estimate` exists. CLI `llmwiki usage` unchanged this PR. Tests: `tests/test_usage_daily.py`, `tests/test_wiki_adoption.py`, extended `tests/test_build_analytics.py`.
