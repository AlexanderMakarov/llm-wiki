...
- **BREAKING — CLI flag `synth --allow-unclassified` (#102)** — harvest now always fails closed. When classification is incomplete the run exits non-zero, names the specific unclassified pages, distinguishes an unreachable backend from an incomplete reply from unreadable source pages, and writes nothing; there is no longer an option to proceed on a guess. The post-write "N filed as `entity_type: unknown`" warning is deleted rather than reworded — it counted every entity stub, not ambiguous ones. Candidate stubs carry no `entity_type`. Scripts passing `--allow-unclassified` must drop the flag and fix the classifier configuration instead.
