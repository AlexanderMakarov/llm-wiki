...
- **Drainable candidate queue (#90)** — `candidates promote` and `discard` accept repeated `--slug`, and `candidates promote --min-refs N` bulk-promotes every pending stub named by N+ distinct source pages (same page-counted evidence as harvest). New lint rule `hollow_reviewed_stubs` flags promoted (`status: reviewed`) entity/concept pages that are still empty stubs; pending pages under `wiki/candidates/` stay exempt (thin is correct while awaiting review).
...
- **`link_integrity` ignores pending candidates (#90)** — pages under `wiki/candidates/` no longer satisfy inbound `[[wikilinks]]`. The gap stays reported until a human (or agent) promotes; generation alone no longer greens the metric. Matches harvest, which already refuses to treat candidates as resolved so re-runs can refresh evidence.
