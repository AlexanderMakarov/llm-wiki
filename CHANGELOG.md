...
- **Demo vault case-collision cleanup + CI guard (#160)** — removed `demo/wiki/candidates/entities/LLMWiki.md`, a stray case-duplicate of the dominant-spelling `demo/wiki/candidates/entities/llmwiki.md`; the two coexisted fine on case-sensitive Linux but collided on case-insensitive filesystems (macOS default, Windows). Added a `lint-and-test` CI step that fails the build when any git-tracked paths fold to the same lowercase form, printing the colliding paths. The harvest writer's exact-spelling wikilink behavior is unchanged; that's tracked in #204.
