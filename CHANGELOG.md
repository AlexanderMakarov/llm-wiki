...
- **Session resume command contrast (#58)** — `.resume-command .resume-cmd-text` now uses explicit `color: var(--text)` on `background: var(--bg-alt)` (beating `.content code`'s `--bg-code`) so axe color-contrast stays WCAG AA.
