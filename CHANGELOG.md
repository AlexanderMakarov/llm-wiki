...
- **BREAKING — `POST /api/candidates` and the Apply controls on `candidates.html` are gone (#109)** — that endpoint lived inside the removed server. **No review capability is lost:** `llmwiki candidates apply --actions` already accepted the identical batch shape, alongside `list`, `promote`, `flip-promote`, `merge`, `discard` and `rewrite-key-facts`. The built page is now a read-only listing of what is pending that prints the exact command to run — including `--vault`, which the old page's copy-CLI line omitted, so it only ever worked against the default vault — plus a ready-made JSON batch covering every listed candidate, with **Copy command** and **Copy JSON** buttons. Edit the batch (`promote` → `flip-promote` / `discard` / `merge`, or delete an entry to skip) and pipe it in: `llmwiki candidates apply --vault <vault> --actions -`.
