...
- **BREAKING — the MCP `wiki_lint` tool returns the `llmwiki lint --json` report (#150)** — the tool was a private reimplementation: **2 hand-rolled checks against the registry's 17**, with a stricter broken-link test (an exact stem match, with none of `_norm_slug`'s normalisation and no anchor stripping) and a narrower orphan test (inbound `[[wikilinks]]` only, against a hardcoded `index`/`overview`/`log` skip list rather than the system-page set) — so it **over-reported on both** relative to what a person running `llmwiki lint` saw, while its own advertised description promised "contradictions, and stale summaries" it had never implemented. An assistant and the person it works for were reading two different accounts of one wiki, and the assistant's was the wrong one. `wiki_lint` now calls the same `load_pages` + `run_lint` the CLI calls and returns the identical payload: **keys `orphans`, `orphan_count`, `broken_links` and `broken_link_count` are gone, replaced by `summary`, `issues`, `total_pages`, `disabled_rules` and `ran`** — findings are a flat `issues` list, each carrying `rule`, `severity`, `page` and `message`. Keeping the old shape alongside the new one was rejected: "the assistant sees what the person sees" is not achievable while there are two shapes to see it in. Expect **findings you have not seen before** — all 17 rules run now, not 2, so contradictions, staleness, frontmatter and catalog health appear for the first time — and expect `disabled_rules`, naming every check your vault switched off in its `llmwiki.json`, plus `ran`, naming the checks that actually produced the payload — between them a short report can never be mistaken for a clean one, whether the run was narrowed by the vault or by the `rules` argument. Two optional arguments mirror the CLI flags: `rules` (a list, or the CLI's comma-separated string) and `min_refs`; there is no `fail_on_*`, because there is no exit code over MCP. An unrecognised rule name is an error rather than a silent skip. Migration for anything reading the old keys: `orphan_count` becomes `len([i for i in issues if i["rule"] == "orphan_detection"])` and `broken_links` becomes the `link_integrity` findings — but note that both are now counted the way `llmwiki lint` counts them, which is fewer, not the same. See [`docs/UPGRADING.md`](docs/UPGRADING.md).
- **`lint` stops calling a deliberate design decision a broken link, and gains `--min-refs N` (#150)** — `synth` writes a `[[wikilink]]` for every topic it names, and the candidate harvest materializes a page only for targets named by **3 or more** distinct source pages; everything below that bar is left unmaterialized on purpose. `link_integrity` was the one component that had never been told, so it reported the entire remainder as broken links — 120 warnings on the shipped `demo/` vault, none of them defects. The rule now honours the same threshold, read from one shared definition (`llmwiki.thresholds.DEFAULT_MIN_REFS`) that the harvest re-exports, and counts references through the same helper the harvest counts with, so the producer and the checker cannot drift. A link to a target named **fewer** times than the threshold is expected and is not reported; a link to a target named **at least** that often with no page still is — that is a genuine gap. `llmwiki lint --min-refs N` states the threshold for a run (default 3); `--min-refs 1` reports every unresolved link, so nothing is permanently hidden, and is the lowest value accepted — `0` and negatives exit 2 on every parser that takes the flag (`lint`, `synth`, `synthesize`, `all`) and on the MCP tool's `min_refs`, rather than silently behaving like 1. Resolution is unchanged: a pending stub under `candidates/` still resolves, and a link to a slug you discarded into `archive/` still reads as broken, on purpose. The demo goes from 120 `link_integrity` warnings to 0 at stock settings.
...
- **Three roadmaps collapsed into one (`docs/roadmap.md`, `docs/public-roadmap.md`, `docs/maintainers/ROADMAP.md`)** — retired in favour of [`context/product/roadmap.md`](context/product/roadmap.md), the roadmap the AWOS loop actually reads. It had drifted into contradicting the product: it still listed the removed HTTP server and `serve.sh` / `serve.bat` as shipped Musts, described highlight.js as CDN-loaded after it was vendored, marked the L2 builder and L3 viewer as "current work", and carried a `Last updated: 2026-04-08` header. Scope decisions it held (Slack/Discord ingest, TUI browser, real-time collaborative editing, a precompiled Go/Rust binary, Sentry/telemetry, a Postgres backend, and the server-side FTS5 search fallback) moved to `docs/maintainers/DECLINED.md`; the five items with anything left to decide moved to the AWOS roadmap's *Later / deferred*. The public and maintainer roadmaps went with it: both advertised capabilities that do not exist — the public page listed hover-to-preview wikilinks and a session timeline view as shipped in v0.2 and an eval framework in v0.3 (#154), and the maintainer page still planned a `.claude-plugin/` marketplace that was deleted in #109 as unworkable. Their one piece of unmet planning, the v1.0 stability goals, becomes *Phase 4 — v1.0 stability pass*; released work is already recorded in this changelog. `docs/index.md`, `docs/architecture.md`, `docs/maintainers/README.md`, `.claude/commands/maintainer.md` and `.claude/commands/triage-issue.md` now point at `context/product/roadmap.md`.
- **BREAKING — `llmwiki consolidate-topics` is retired (#147)** — the subcommand name stays registered so typing it still resolves, but it always exits `2` with a message that synthesis now prepares the known-names list. It does not emit a prompt, accept `--complete` as a lifecycle step, or write `.llmwiki-topics.json`. Duplicate spellings are avoided by the known-names prepare job at the start of each `synth` run. Agent kit `/wiki-synth` no longer tells agents to run it.
- **`.claude-plugin/` (`plugin.json`, `marketplace.json`) (#109)** — the Claude Code plugin packaging could not work: `path: "."` resolved commands to a directory that does not exist, the Python floor was listed as 3.9 against an actual 3.12, and most commands were missing. Delivery is the package plus `llmwiki install-agent-kit --dest PATH`.
- **BREAKING — `llmwiki serve` and the `serve.sh` / `serve.bat` helpers are gone (#109)** — the product no longer ships a command that starts an HTTP server, and `llmwiki.serve.serve_site` is no longer part of the public API. A built site is plain files: **open `site/index.html`** (or `<vault>/site/index.html`) in a browser and navigation, project pages, session pages, topic pages, search and the graph all work with nothing running. Publishing to a static host is unchanged — see `docs/deploy/`. If you want an HTTP origin for a local check, `python3 -m http.server --directory site` is a stdlib stand-in. The Docker image follows: no `EXPOSE`, no ports in `docker-compose.yml`, and its default command is `build`, writing into the bind-mounted `site/` you then open on the host.
- **BREAKING — `POST /api/candidates` is gone (#109)** — that endpoint lived inside the removed server. **No review capability is lost, on the page or off it.** `/candidates.html` still opens with two review tables — **Entities (pending)** and **Concepts (pending)**, columns Name / Description / **Decision** — and a per-row Decision control offering exactly the actions `llmwiki candidates apply` executes: *Promote*, *Flip and promote*, *Merge into…* (a filterable dropdown over the pages `merge --into` resolves — the trusted pages under `wiki/<kind>/` then the same-table pending stubs; open it with the ▾ button or `↓` to browse every target, type any part of a name to narrow it, `↑`/`↓` and `Enter` to choose, `Esc` to close) and *Discard* (with a required reason, which `discard` files beside the archived stub). Both of those fields must be resolved before the row can be applied: an unlisted merge target is marked as you type, and a row that is decided but not yet executable is named on the page and its field focused, instead of entering a batch that would fail at the CLI. Deciding is browser state, so the page needs nothing running and works opened from `file://`. Every row starts at **No decision**: only the rows you actually chose enter the batch, and a row left undecided stays pending, so a half-finished review can be applied and resumed. **Apply** assembles those rows into the command to run and the JSON batch to pipe into it — `llmwiki candidates apply --vault <vault> --actions -`, naming the vault so the batch cannot land in the wrong knowledge base — with **Copy command** and **Copy JSON** beside them. The Apply bar sits above the tables and stays pinned under the nav while you scroll, so it is in reach from any row. Executing the decisions is the one step that was ever a server's job, and it is the command line's: `list`, `promote`, `flip-promote`, `merge`, `discard`, `apply` and `rewrite-key-facts` all accept the same batch shape the page prints.
- **`/wiki-serve` slash command** — deleted along with the subcommand it wrapped. `/wiki-build` now tells you to open the built site.
- **Nothing in the project serves the site for its own purposes either (#109)** — the browser suite reached the site over HTTP throughout the server's removal, so every check passed and none exercised the claim. The `ThreadingHTTPServer` behind `tests/e2e/` is gone and its `base_url` fixture is replaced by `site_url`, a `file://` prefix over the built output, plus `site_has()` for "did the build emit this page"; `playwright.config.ts` carries no `baseURL` and no `webServer` block, and `tests/agents/` specs address pages by file URL under `LLMWIKI_SITE_DIR`. `agents-e2e.yml` and `synthetic.yml` open the built files instead of starting `python3 -m http.server` in front of them, `.claude/launch.json` opens `site/index.html`, and `scripts/record_demo.py` (`--site`) and `scripts/regen_docs_screenshots.py` capture from files. New `tests/e2e/test_demo_walkthrough.py` builds the committed demo vault and walks home, projects, a project, sessions, a session, a topic page, the graph, candidates and search from disk, failing on a console error or a site file that does not load; `tests/test_harness_starts_no_server.py` keeps a server from coming back into the harness.
...
- **`synth --estimate` and `synth` share one done predicate (#163)** — estimate treated a source as already synthesized when its wiki pages existed on disk (or appeared in discovered source keys), while a real run skips only when synth state has a fresh-enough mtime entry and the page is not pending. Restored pages, cleared state, or a multi-part doc interrupted mid-write made estimate report `$0` for work the next run would still bill. Both paths now call `source_synth_is_done()` (state + mtime + pending, same `1e-6` tolerance). Pages on disk alone no longer count as done; the unused `synthesized_source_keys=` kwarg on `synthesize_estimate_report` is gone (test callers that injected it can drop it). The separate #37 dedup-elsewhere skip is unchanged.
- **A failing `lint` now records that it ran (#150)** — `cmd_lint` returned on `--fail-on-errors` *above* the `last_lint_run_at` state update, so the runs most worth recording were the only ones never recorded, and the Home "last lint" timestamp silently stood still while the gate was red. The exit code is computed last, so neither `--fail-on-errors` nor the new `--fail-on-warnings` can skip the update.
- **`lint` reports the same findings in the same order every run (#150)** — several rules walk a `set`: `link_integrity` iterates `llmwiki.wikilinks.wikilink_targets`, which returns one, so with `PYTHONHASHSEED` randomised two *processes* listed the identical findings in different order, and a report was reproducible only within one interpreter. Harmless while lint was one command, and not harmless once the MCP `wiki_lint` tool and the CLI were required to be the same report for the same wiki — they are different processes, so a raw diff of the two payloads disagreed about ordering alone, which reads as a difference to anyone comparing them. `run_lint` now sorts each rule's findings by `(page, message)` before collecting them, keeping the registry's rule order, so an unchanged vault produces a byte-identical report run after run, across processes, and through either route.
- **Shipped wiki-all skill matches `llmwiki all` (#170)** — `llmwiki/agent_kit/skills/wiki-all/SKILL.md` no longer teaches init→sync→graph→build→lint, omits synth, hardcodes an Obsidian export path, or quotes a fixed lint-rule count. It mirrors `/wiki-all` / `python3 -m llmwiki all`: sync → synth → build → graph → lint, every registered lint rule, no home vault paths.
- **A scheduled job installed against a non-default vault now runs against that vault (#156)** — `install-automation --vault <path>` wrote its status file under `<path>` but scheduled a bare command, which resolved its vault from `vault.default_path` in `config.json`. The scheduled command now carries `--vault <path>` whenever the install targets a vault other than the configured default; a default install emits the same command line as before.
- **An ingest job no longer records extras it will never run (#156)** — passing `--graph` / `--lint-fail` with `--job ingest` was noted as ignored but still written to `.llmwiki/automation-status.json`, so the Home Automation panel could advertise a failure policy for a job that runs no lint step. Both normalise to `none` / `never` for any job but maintain, so the status file, the label, the panel and the scheduled command describe the same job.
- **Scheduler files survive a path containing `&`, `<` or `>` (#156)** — the launchd plist and Windows Task Scheduler XML interpolated the wrapper, working directory, and log paths unescaped, so a vault or home directory holding an XML metacharacter produced a file the scheduler rejected with an opaque parse error.
- **`synth --estimate` and `synth` agree on which documents are pending (#161)** — the estimate rebuilt each document's `<name>--part-NN.md` page list from the chunk size in effect *now*, while the run globs the pages actually on disk, written under whatever chunk size was in effect *then*. Whenever the two differ — a tuned constant, a chunker change, or a document edited near a boundary — the estimate priced a different set of sources than the run would process, in both directions: a part page the current chunk count does not reach was invisible, so a document still needing work was reported as done and billed at nothing, and a document fully covered by the parts on disk was billed for a part that does not exist. Since the estimate is the only thing standing between you and an unattended spend, that was a wrong dollar figure rather than a cosmetic mismatch. Document target pages now resolve through the same `source_page_paths()` disk walk the run uses, so stub and topics-rewrite-pending parts are seen wherever they sit. Cost is unchanged and still scales with the chunk count the **next** run will produce, because a pending document is re-chunked at the current size. Sessions are unaffected — they are never chunked and already agreed. No migration: the next `synth --estimate` is correct.
- **`install-automation` profile B runs `synth`, not deprecated `synthesize` (#147 / #90)** — profile B was still chaining `synthesize`, which defaults to sources-only and skips candidate harvest. It now runs `synth` (sources + harvest) before `build`, matching the canonical loop. Profile C was already on `all --with-synth`.
- **Ctrl+C during `synth` harvests from written pages; Home `on_disk` recovers on `build` (#145)** — interrupting a sources pass drains in-flight pages, then collects pending names from what landed on disk (unless `--sources-only`, which prints `llmwiki synth --candidates-only` instead) and exits **130**. `build` refreshes Home pipeline `on_disk` when stored totals disagree with `wiki/sources/**/*.md`, so synthesized counts do not stay at zero after an interrupt until an estimate run.
- **Discarding a candidate no longer breaks `lint`, and a dismissed term stops coming back at you (#140)** — dismissing a candidate means judging the term to be noise (a tool name, an example, something repeated by accident), but `wiki/archive/` was read nine different ways: `lint`, `backlinks` and `tags` skipped it while `reindex` catalogued it, `graph` and `graphify_bridge` emitted its pages as nodes, and the MCP `wiki_search` / `wiki_query` / `wiki_lint` tools searched, quoted and link-resolved it. So `llmwiki candidates discard` wrote an `## Archive (N)` section into `wiki/index.md` whose bullets `lint` then reported as `index_sync` dead index links — `lint --fail-on-errors` exited 1 on a correct vault, failing the `wiki-checks` workflow, and deleting `wiki/archive/` did not help because the orphaned heading survived as unmanaged free text. Meanwhile `wiki_search` still returned discarded stubs as hits, and `wiki_lint` resolved `[[wikilinks]]` against archived slugs that `llmwiki lint` called broken, so the two disagreed about the same vault. `archive/` is now cold storage behind one predicate (`_system_pages.is_archived_path`, which `candidates.py` — its only writer — also builds its destination from): never catalogued, never linted, never a graph node, never tagged, never given backlinks, and never searchable or quotable through MCP. **Existing vaults self-heal** — a leftover `## Archive` section and its bullets are dropped by the next `reindex`, which `sync`, `synth`, `remove` and the `candidates` actions all run; there is no migration to apply, though a note you hand-wrote under that heading goes with the section. Three deliberate consequences: the candidate harvest **still** counts archived slugs as resolved — the archived stub is the only record that you dismissed the term, so without it every dismissal would be re-proposed on every `synth`, forever; `[[wikilinks]]` pointing at a discarded slug stay broken and are **not** rewritten (the target was thrown away on purpose, so the link wants a human decision); and the rule matches **only** top-level `wiki/archive/**` — a folder named `archive` nested under a page folder, such as a project slug of that name, remains a live page set rather than silently vanishing from the index.
- **Home On disk column stuck at 0 after upgrading past #81** — vaults that already had a shape-ok `synth.pipeline` (raw / pending / synthesized) never re-ran the estimate walk on `build`, so pre-`on_disk` rows persisted and the widget's `Number(row.on_disk || 0)` showed zeros even when `estimate.source_pages_on_disk` was correct. `build` now refreshes when any pipeline row lacks `on_disk` (or rows are empty while pages exist on disk); rows that already carry the key keep the cheap skip path.
- **`build` no longer rescans `wiki/sources/**` for every session/document page (#122)** — provenance links used a full `rglob` + frontmatter parse per page (`find_wiki_source_for_raw`), which turned large vault builds into hours of quadratic I/O. Build now constructs a `source_file:` → wiki path index once per run and does O(1) lookups; the public helpers accept an optional `index=` (process-local cache when omitted).
- **Document Sources hrefs match `documents/` layout (#122)** — prefer-HTML for `source_file: raw/docs/…` now uses `documents/{rel}.html` (same as `RawDocFile.out_rel`), including flat files that previously became `documents/{project}/…` or `None`. Document builds also copy the source `.md` beside the HTML so `exclude_href` on the page can fall back to a `(raw)` new-tab link, matching session FR2 behaviour.

### Added

- **`llmwiki migrate-topic-kinds --vault PATH` (#174)** — offline one-time stamp of missing `(entity|concept)` labels onto older source Connections bullets from pages already under `wiki/entities/`, `wiki/concepts/`, and matching `wiki/candidates/` folders (no synthesis backend or network call). Complements the #147 catch-up: clears the topics-rewrite flag when at least one resolvable kind can be stamped, without inventing `fact:` lines. Also upserts synth state for raw sources whose wiki targets are rewrite-clear (including shared synth filenames) so plain `synth` / `--estimate` do not re-bill them after #163. `--dry-run` previews; a successful apply writes vault-root `.llmwiki-topic-kinds-stamped.json` so a later `synth --force --path …` can target exactly those `source_file` entries. Docs: `docs/UPGRADING.md` (beside #147), `docs/reference/cli.md`.
  - *Release note:* Offline `migrate-topic-kinds` stamps missing topic kinds and marks rewrite-clear sources done in synth state (#174).
- **A wiki can declare the quality checks that do not apply to it, and every report says what it skipped (#150)** — a committed `<vault>/llmwiki.json` carries `lint.disabled_rules`, either as a list (`["content_freshness"]`) or as an object mapping each rule to a written reason (`{"content_freshness": "a committed snapshot ages by the calendar"}`). The declaration lives with the wiki, so copying, sharing, or publishing it carries the opt-outs along; it switches a check off or on and cannot re-grade a finding's severity. A disabled rule never runs and contributes nothing — and is **named as skipped in every report, with its reason, whether or not anything was found**, so a clean result can never quietly mean "almost nothing was checked". `lint --json` carries the same list as `disabled_rules`. Switching off every registered rule reports that nothing was checked rather than printing a clean summary — counted against the rules the run would have used, so `--rules content_freshness` on a wiki that disables it says 1 rule of 1 was skipped, not 1 of 17. A misspelled or retired rule name stops the run with exit 2, naming the file, the unrecognised entry and the valid names, rather than leaving a check switched on that you believed you had switched off — and an `llmwiki.json` that cannot be parsed is the same hard error, never a silent "no opt-outs". A vault that declares nothing behaves exactly as it did before.
- **`lint --fail-on-warnings` (#150)** — warning-severity findings stop the gate as well as errors, beside the existing `--fail-on-errors` (which is unchanged, and still the default posture: warnings are reported and exit 0). Mirrors `all --lint-fail warnings` for the standalone command. Checks a vault switched off in `llmwiki.json` cannot stop the gate, because they never ran.
- **`llmwiki all` stage opt-outs and a lint failure policy (#156)** — `--no-sync`, `--no-synth`, `--skip-lint` (joining the existing `--skip-graph`), and `--lint-fail {never,errors,warnings}`.
- **`install-automation` non-interactive flags (#156)** — `--job {ingest,maintain}`, `--graph {none,builtin,graphify}`, `--lint-fail {never,errors,warnings}`, and `--schedule "<cron>"` cover every wizard answer, so an unattended install produces the same scheduled command as answering the questions. `--profile {A,B,C}` is deprecated in favour of `--job` (`A`→`ingest`, `B`/`C`→`maintain`) and prints a notice; `--job` wins when both are given.
- **`llmwiki/cron_spec.py` and `llmwiki/automation_plan.py` (#156)** — two dependency-free modules that own the vocabulary of *when the job runs* and *what the job does*, so the wizard, the scheduled command, the status file, and the site's Home panel are all generated from one definition each.
- **`llmwiki install-agent-kit --dest PATH` (#109)** — copies the user-facing `/wiki-*` slash commands and skills from the installable package into an agent directory you name. A pip or Homebrew install now carries the files; cloning this repository is not required. `--dest` is required (no auto-detection). Re-running after an upgrade refreshes copies; a conflicting file whose content differs is saved as `<file>.bak` beside it and reported, an identical file is left alone, and `--dry-run` writes nothing. Source of truth is `llmwiki/agent_kit/{commands,skills}/`. Contributor-only commands and skills stay in `.claude/`. Docs: `docs/reference/cli.md` (`## install-agent-kit`), `docs/UPGRADING.md`.
- **`scripts/refresh_demo.py` (#109)** — a local maintainer command that incrementally refreshes the demo vault from git-detected product-doc changes (`docs/`, not `docs/maintainers/`), then runs `synth --docs-only`, `build`, and `lint` against `demo/`. Documented in [`docs/maintainers/REFRESH_DEMO.md`](docs/maintainers/REFRESH_DEMO.md). Requires a git working copy and a reachable synthesis backend; it cannot run from a release archive and never runs in CI.
- **Page-kind reference (#109)** — [`docs/reference/page-kinds.md`](docs/reference/page-kinds.md) describes every surviving `type:` value (`source`, `entity`, `concept`, `project`, `synthesis`, plus the system kinds `navigation` and `context`): what each is for, a demo example, and a provenance (`synth` / `harvest` / `build` / `human`) for every frontmatter field, derived from the producers in code. Saved answers are documented as agent- or person-written, never generated automatically. Linked from `docs/index.md`.
- **The built site fetches nothing (#109)** — `highlight.min.js` and both GitHub themes (`github.min.css`, `github-dark.min.css`, pinned at 11.9.0) are vendored under `llmwiki/vendor/` and copied into the site root on every build, referenced by relative path exactly the way the vendored vis-network bundle already was (#127). Opening a built site offline now gives the same result as opening it online, and `pyproject.toml` packages `vendor/*.css` alongside `vendor/*.js` so pip and Homebrew installs ship the themes rather than the library alone. Attribution is recorded in `llmwiki/vendor/NOTICE`.
- **`llmwiki build --local-root PATH` (#109)** — the value shown in place of a session's stored home directory is now a build input. It resolves from the machine running the build by default, so browsing your own site locally still gives paths you can paste into a shell with no configuration, and a fixed string makes the same vault render identically anywhere: `llmwiki build --vault demo --local-root /home/user`. The demo publish (`pages.yml`) passes one, so a published page never shows whoever happened to run the build.
- **`llmwiki migrate-page-kinds --vault PATH` (#109)** — the supported way off the removed `question` / `comparison` page kinds. Each page carrying one is retyped to `concept` and moved into `wiki/concepts/` **keeping its filename**, the legacy `wiki/questions/_context.md` and `wiki/comparisons/_context.md` are deleted, and each folder is pruned once empty. Inbound `[[wikilinks]]` are deliberately not rewritten: resolution keys on the filename, never the folder, so a relocated page keeps every inbound link and no referring page is touched. Nothing is overwritten and nothing unrecognised is deleted — a filename already taken in `wiki/concepts/` means the page is retyped where it stands and reported as a collision, and a legacy folder still holding other content is left in place and reported. `--dry-run` previews the same per-file report while writing nothing; a vault with no such page prints `nothing to migrate` and exits 0. A run that changed something reconciles `wiki/index.md` and appends `## [YYYY-MM-DD] migrate | page kinds` to `wiki/log.md`. Unlike the other `migrate-*` commands the implementation lives in the package (`llmwiki/migrate_page_kinds.py`) rather than under `scripts/`, so it runs from a pip or Homebrew install with no checkout on disk.
- **Trace a wiki page down to raw transcripts (#122)** — `llmwiki trace <page>` walks the encoded provenance chain (`sources:` → `source_file:` → `raw/…`) and prints each hop (role, title, location), marking missing hops without inventing body excerpts. Exit non-zero only when the starting page cannot be resolved. Lint gains `provenance_integrity` (**error** severity): one finding per broken hop on pages that already carry provenance metadata; pages without those fields are skipped. Guided repair belongs to `doctor` (#110) — this release only reports. On the built site, session and document pages link provenance Sources entries to preferred built HTML when available, otherwise to the raw file (or site copy) with a visible “(raw)” mark and `target="_blank"` `rel="noopener"`. Topic pages do **not** repeat that provenance panel: their evidence list is a collapsible **Sources** section (shared Home/Analytics chrome) split into **Sessions** vs **Documents** by compiled URL. No new MCP tool — use the CLI (or existing page/read tools) for the chain. Docs: `docs/reference/cli.md` (`## trace`, Rules), `docs/UPGRADING.md`.
- **`synth` announces the batch and synthesizes pages in parallel (#118)** — a real run now prints what it is about to do before the first page is called: `Synthesizing 11 source(s) with ClaudeCLISynthesizer (2 at a time)`, where the count is the work queue after up-to-date, ineligible, and already-claimed sources are excluded — an empty queue says `Nothing to synthesize — every source is already up to date.` rather than going silent. Pages are then synthesized **two at a time by default** instead of strictly one after another, so a catch-up run's wall clock shrinks roughly in proportion; the waits being overlapped are backend I/O, not local work. Each result line carries its position, `  [3/11] synthesized: <project> → <page>`, counting completed sources against the announced total — pages finish in whatever order the backend returns them, so positions arrive out of order while the last is always `N/N`. Error and `protected:` lines carry a position too. The count is settable per run with `llmwiki synth --concurrency N` (range 1–16; `1` restores strictly sequential behaviour) or persistently with `synthesis.concurrency` in `config.json`, which is also what `all --with-synth` reads; the flag wins when both are set. An unusable or out-of-range config value warns and falls back to the default rather than failing a scheduled run, while a bad `--concurrency` on the command line exits `2` with the accepted range. **Nothing else about the result changes**: pages, skip decisions, stub protection, the single log entry with its producer breakdown, and the candidate harvest that follows all match a sequential run for the same inputs, verified by an equivalence test. A source is recorded as done only after its page is written, so an interrupted run resumes at exactly the remainder, and one failing page still leaves the rest of the batch to complete. An abandoned run cancels the pages still queued rather than working through the rest of the backlog first — that covers Ctrl-C and any other failure escaping the run's bookkeeping — and every page that already reached disk is recorded, so the resume does not synthesize and re-bill it; Ctrl-C additionally reports how many pages are still in flight instead of appearing to hang. `ClaudeCLISynthesizer` token and cost accounting is now lock-guarded, so concurrent pages cannot under-report spend, and `BaseSynthesizer.synthesize_source_page` documents the thread-safety contract any future backend must honour.
- **Topic pages say what a topic is, how current it is, and what the wiki records about it (#108)** — `topics/<slug>.html` opens with an identity line under the title: a kind chip (Entity · Concept · Project · Synthesis · Source), the freshness dates, the connected-topic and session counts, and the page's slug. Freshness is two separate facts and is never invented: `Active <first> – <last>` comes from the dates of the sessions that mention the topic, `Reviewed <date>` from the backing wiki page's own `last_updated`. A topic no wiki page describes — an un-promoted candidate, or a name a reviewer declined — is labelled `Unclassified topic` and shows no review date and no content, with no placeholder, empty heading, or blank field anywhere; the chip is never dropped, because the absence of a page is itself a fact and a missing chip would leave a reader unable to tell an unclassified topic from a page that failed to render one. **Entity and concept page content now reaches a reader for the first time**: the topic page renders the backing page's body above the connections — everything after its `# H1` except `## Connections` and `## Sessions`, which the topic page renders itself from the graph. It is heading-agnostic rather than keyed to `## Key Facts`, so renamed, reordered, or newly added sections and any introductory prose all survive as written; a heading left with nothing under it is dropped rather than shown empty; and `[[wikilinks]]` inside resolve to topic pages or compiled session pages, degrading to plain text when they resolve to nothing rather than becoming dead links. **Project topics route to the project page** (`projects/<slug>.html`) instead of a thin topic page — from the map, the search index, connected-topic lists on other topic pages, `topics/index.html`, and citations inside page content — including when the spelling sessions used is an alias of the project's own name. A project page that exists in `wiki/projects/` but was never compiled (no recorded sessions) keeps its ordinary topic page rather than being handed a link that 404s. **Project pages gain a Connected topics list** in the same shape as on topic pages, immediately above the session tables and omitted entirely when there is nothing to list, plus a `Project` kind chip in the hero — the same chip the topic page would have shown, so routing a reader here costs them nothing — plus `created` / `updated` in the hero derived from the project's oldest and newest session — project stubs carry no date of their own, so this is recomputed each build and stays correct as sessions arrive. **Every topic kind gets its own colour in the map**: projects magenta `#db2777` and topics no page describes lime `#65a30d`, joining the existing violet / blue / green / amber — those kinds previously shared one fallback colour, so the legend offered distinct labels against a single swatch. Red stays reserved for the orphan border and the live search match, which are signals rather than kinds. The **graph side panel** carries the same `Kind`, `Active` and `Reviewed` rows beside the counts it already showed: `Kind` always renders, naming the unclassified state like the page's chip, while each date row is dropped when the node lacks that field. **The map stops opening new tabs except on double-click** — the side panel's `Open page →`, its session links, and the right-click menu's **Open** navigate in the current tab like every other link in the site, leaving double-click as the one deliberate "take this elsewhere" gesture. **Search results name the kind**: a topic entry in `search-index.json` gains a `kind` field carrying the same singular label the chip uses, and the ⌘K badge reads it instead of the generic `Topic`, so a search hit, the map and the topic page all agree. The entry's `type` is unchanged and stays `topic`, so the documented `type:topic` palette filter still matches every topic result — an additive index change, not a contract break. Documented in [`docs/reference/ui.md`](docs/reference/ui.md), which gains a Topic pages section naming which facts come from sessions and which from the topic's own wiki page. **No migration and no re-sync**: every new field is derived at build time from frontmatter already on disk or from the file's own location, so the pages appear on the next `llmwiki build`. One behaviour change rides along with the `[[wikilink]]` parsing consolidation this needed: a page that links the same target twice, once bare and once with a `#section` anchor (`[[a]]` and `[[a#x]]`), now counts as one reference rather than two — so `llmwiki references <entity>` lists it once, a rendered `## Referenced by` block lists the referring page once, and the `orphan_detection` lint rule counts it once.
- **BREAKING — `project` is a first-class page kind (#102)** — `type: project` joins `entity` and `concept` in the set `frontmatter_validity` accepts, so project pages stop declaring `type: entity`. `build --seed-project-stubs` writes `type: project` (and no `entity_type`), `claim_verification` covers project pages, and the graph relevance bonus applies to them — gated on a non-empty `file` so synthetic project hub nodes stay unboosted. The project pages tracked in this repository are migrated in the same change. Vaults elsewhere keep working unchanged: `type: entity` on a project page is still valid, and pages that already carry `entity_type` keep it as inert metadata. Breaking for anything that assumed every page under `wiki/projects/` reports `type: entity`.
- **AWOS context PR gate (#117)** — governance CI fails when a PR changes `llmwiki/`, `integrations/`, `tests/`, `.github/workflows/`, `docs/maintainers/`, or `docs/reference/` without also changing something under `context/`. Path filters alone decide; there is no label bypass. Tutorials, scripts, packaging, guides, examples, and similar areas stay exempt.
- **Cursor-compatible AWOS workflow (#114)** — Layer A file-copy + flat `/awos-*` wrappers; Layer C via `./scripts/sync-awos-plugin-cursor.sh` / `update-awos.sh --plugin` (acplugin + **`awos-` prefix relocate** so `/awos-flow` resolves under `.cursor/commands/`); `AskQuestion`-first runtime rule; dual MCP; maintainer docs. Whole `context/` tree is tracked (product/spec working docs). Recruited AWOS hire set under `.claude/skills/` / `.claude/agents/` plus `fix-bug` / `implement-feature` commands. Repo-root acplugin dumps gitignored. Companion research note [`docs/maintainers/AGENT-WORKFLOW-ALTERNATIVES.md`](docs/maintainers/AGENT-WORKFLOW-ALTERNATIVES.md) compares OpenSpec, Spec Kit, GSD, BMAD, Superpowers, and Trellis on Cursor OOB support, features, and learning curve.
- **Candidates review UI (#97)** — dedicated `/candidates.html` with pending Entities / Concepts tables. Each row sets a decision (Skip · Promote · Flip and promote · Discard · Merge); **Apply** runs a batch. Under `llmwiki serve`, Apply POSTs `{"actions":[…]}` to `/api/candidates` (shared library paths, including Key Facts on promote from #103 and catalog reconcile from #101). On a static / `file://` open, Apply shows one pasteable `llmwiki candidates apply --actions '…'` command (same JSON). Header nav places **Candidates** immediately after **Raw**. Home Knowledge-layer **Candidates** and Analytics **Candidates to review** link there; `/wiki-candidates` documents the same actions; no hand-`mv` flip path.
- **Promote fills empty Key Facts from source `fact:` bullets (#147, supersedes #103)** — `llmwiki candidates promote` copies facts already recorded on source Connections; it does not call a model. Non-empty reviewer Key Facts are preserved. `rewrite-key-facts` remains the opt-in LLM recovery for trusted pages that still carry machine-assembled Key Facts (or pasted harvest-stub `## Candidate merge` blocks). `--slug` one page, or `--all` for every entity/concept.
...
- **BREAKING (behaviour change) — `llmwiki all` runs every stage, including the previously opt-in `sync` and `synth` (#156)** — the pipeline is `sync` → `synth` → `build` → `graph` → `lint`, and each stage now has an opt-out flag instead: `--no-sync`, `--no-synth`, `--skip-graph`, `--skip-lint`. **An existing bare `llmwiki all` in a scheduled job starts summarising sessions after this upgrade.** The default `synthesis.backend` is `dummy`, which makes no provider call, so a default install spends nothing — the change only reaches users who configured a real backend, and those users get a one-shot terminal notice before the first provider request. `--no-synth` is the opt-out. `--with-sync` / `--with-synth` still parse so already-installed scheduled commands do not die on "unrecognized arguments", but they are inert and each prints a deprecation notice; `--no-*` wins over `--with-*` in any order. See [`docs/UPGRADING.md`](docs/UPGRADING.md).
- **BREAKING (behaviour change) — lint findings no longer fail `all` by default (#156)** — the new `--lint-fail {never,errors,warnings}` decides whether wiki-quality findings end the run with exit `2`, and the default is `never`: lint still prints its full report and the run still exits `0`, so a red scheduled job means the job actually broke. `--strict` is retained as the spelling for `--lint-fail warnings`, and when both are given the stricter wins.
- **`install-automation` asks one plain-language question instead of a three-letter profile menu (#156)** — the wizard asks what the daily job should do (*Ingest only*, which never contacts an AI provider, or *Maintain*, which summarises sessions and costs money once a real provider is configured), naming the folders each writes; then offers the optional extras (knowledge graph with a builder choice, fail-on-errors, fail-on-warnings), then a schedule picked from presets (Every day / Weekdays only / Once a week / Custom cron); then prints the job in words, the run time, and the exact command line, and writes nothing at all if you decline. Every prompt re-asks on invalid input rather than silently defaulting. The scheduled command is a single `llmwiki all …` run rather than a chain.
- **`install-automation` schedules with cron expressions (#156)** — `--schedule "0 8 * * 1-5"` replaces `--hour` / `--minute` as the source of truth and is what makes "weekdays only" and "once a week" expressible; the expression is translated to systemd `OnCalendar`, launchd `StartCalendarInterval`, or a Windows Task Scheduler trigger. Standard 5-field cron only — nicknames (`@daily`), Vixie/Quartz extensions (`L`, `W`, `#`), a seconds field, and any expression restricting both day-of-month and day-of-week (which cron ORs and no OS scheduler can express) are refused with exit `2` and the reason. `--hour` / `--minute` are deprecated, still accepted, and ignored with a notice when `--schedule` is given.
- **The Home Automation panel names the job in words (#156)** — `Maintain + graph (built-in)` and `Weekdays at 08:00` instead of a bare profile letter and a raw `HH:MM`, plus a line stating whether the job can spend money at an AI provider and, when a failure policy is set, that quality findings can mark it failed. A status file written by an older version (letter only) is read through the legacy map rather than rendering blank.
- **Home Commands table leads with automation (#156)** — `llmwiki install-automation` and `llmwiki all` are on the built site's copy-paste command list, so the Home page stops presenting llmwiki as a list of daily chores.
- **Newcomer docs point at automation instead of a daily routine (#156)** — [`docs/cheatsheet.md`](docs/cheatsheet.md) presents the daily job as the normal path with the manual commands as the one-off alternative, and [`docs/getting-started.md`](docs/getting-started.md) / [`docs/tutorials/00-quickstart-walkthrough.md`](docs/tutorials/00-quickstart-walkthrough.md) offer automation as the next step after the first successful build. Stale cheatsheet facts fixed along the way: the Graphify install line said `pip install llm-notebook[graph]` (the distribution is `llm-wiki`), a heading claimed "11 CLI commands" over a 12-row table when `llmwiki --help` lists 25, and the page gave the lint rule count as both 14 and 17 (it is 17).
- **One synthesis pass per source — topics, kind, facts, and description together (#147)** — each `synth` run makes **two** language-model jobs, then bookkeeping: (1) prepare known-names once at start (canonical name, aliases, kind, short description) from wiki already on disk — Dummy / `not is_llm` skips this and uses heuristic vocabulary inject; (2) one source-summary ask per queued raw file, with that frozen list in the prompt. Connections bullets carry kind plus nested `fact:` claims. Harvest and promote are parsers plus a file move — no model. Promote fills empty Key Facts from source `fact:` bullets (reviewer Key Facts preserved) and works with Dummy/`None`; `rewrite-key-facts` still requires an LLM. Next synth rewrites source pages that lack parseable topic bullets (FR2), then skips when up to date. Custom `wiki/prompts/source_page.md` must match the new Connections shape. Merge stitching stays #148. Docs: `docs/reference/cli.md`, `docs/reference/synthesis-cost.md`, `docs/UPGRADING.md`.
- **`candidates apply` rebuilds the static site after a successful batch (#109)** — promoting, merging, or discarding used to update `wiki/` while the open `candidates.html` still listed those rows, because nothing served a reload. `apply` now rebuilds `site/` beside the wiki so a reload of the file matches the queue. `--no-rebuild` skips that when you are applying several batches before one `llmwiki build`. One-off `promote` / `merge` / `discard` still do not rebuild. Docs: `docs/reference/cli.md`, `docs/reference/ui.md`.
- **The published demo includes a synthesized wiki layer (#109)** — `demo/wiki/` ships source summaries, promoted entities and concepts, and pending candidates so GitHub Pages (`pages.yml` / `build --vault demo`) is a knowledge base, not only raw sessions and product docs. A full `refresh_demo.py` regen is still a maintainer step, not CI.
- **Displayed session paths no longer undo the import-time redaction (#109)** — the site used to reverse the username substitution applied during `sync`, which coupled two functions that had to agree forever and made a build depend on the machine that ran it. It now rewrites the home directory a stored `cwd` starts with to the build's local root (see `--local-root` above), including the matching dash-encoded segment agent stores use (`-Users-<user>-code-x`). **Substitution is restricted to the `cwd` field**: a session description is prose from the first user turn and is rendered exactly as imported, where before arbitrary path-shaped text inside it was rewritten. `llmwiki.convert.restore_local_path` is removed; the path-shape allowlist it shared with redaction is now `llmwiki.convert.HOME_PATH_PREFIXES` / `ENCODED_PATH_PREFIXES`.
- **An llmwiki source checkout can no longer be used as a knowledge vault (#109)** — a git clone carries a `.llmwiki-source-checkout` marker at its root, and the subcommands that write vault content (`init`, `sync`, `synth`, `synthesize`, `add`, `build`, `all`, `watch`) refuse to run against a directory carrying it when no vault was named, exiting `2` with a message pointing at `--vault` and at the repository's own `demo/` vault. Contributors previously grew `raw/`, `wiki/` and `site/` inside the clone by running a bare command; the example vault now lives in `demo/` and scratch runs need an explicit `--vault`. **Installed distributions are unaffected** — pip and Homebrew unpack the package beside no marker, so the guard never fires for a user, and naming a vault (`--vault PATH`, or `vault.default_path` in `config.json`) bypasses it everywhere.
- **UI surface specs live under the maintainer docs (#109)** — the ten hand-written page-type specs are at [`docs/maintainers/surfaces/`](docs/maintainers/surfaces/README.md) and are listed in the maintainer guide's docs table, so a reviewer finds them next to `REVIEW_CHECKLIST.md` rather than in a top-level directory whose name collided with the AWOS `context/spec/` working tree. ADR-001, the Playwright agents bootstrap note, and `tests/e2e/features/regression.feature` point at the new path.
- **Local delivery review is one independent pass + chat** — `/implement-feature` and `/fix-bug` no longer run a dual checklist/reviewer pair. One independent review (the coding agent picks its own most suitable review skill or command) loads `REVIEW_CHECKLIST.md` plus the companion maintainer docs (Blocker-vs-nit priority), writes `review.md`, and the orchestrator prints the full review in chat for keep/drop before push. Decision record: `context/product/delivery-flow.md`.
- **Knowledge Graph loads companion assets and works offline from the built static site (#127)** — the interactive viewer no longer embeds its full script in `graph.html` or pulls vis-network from unpkg at view time. `llmwiki graph` and every `build` now emit three siblings beside each other: `graph.html`, first-party `graph-viewer.js`, and pinned `vis-network.min.js` (**9.1.9** UMD, vendored under `llmwiki/vendor/` with attribution in `NOTICE`, shipped in the wheel via setuptools `package-data`). Graph data stays a tiny inline `const GRAPH = …` stub, so opening `site/graph.html` over `file://` or fully offline renders the canvas without a CDN fetch or a manual host step for vis-network. The inline offline notice appears when either companion script is missing or fails to load (HTML `onerror` + load watchdog on `graph-viewer.js`, plus a `typeof vis` check inside the viewer for the library) — not merely when the network is down. Documented in [`docs/reference/ui.md`](docs/reference/ui.md) and [`docs/reference/cli.md`](docs/reference/cli.md).
- **Honest Corpus / Already synthesized counts (#81)** — `synth --estimate` and the Home Pipeline table label totals as **eligible sources** (synthesize inputs), not pages or files under `wiki/sources/`. Corpus reads `N eligible sources (S sessions + D docs)`; Already synthesized reads `N of M eligible sources` (the old `pages in wiki/sources/` framing is gone). CLI prints **Source pages (current state): T on disk (Sess sessions + D docs + X stubs)** as `.md` file counts (not unique `source_file` keys). Home's eligible-sources table gains an **On disk** column (per-agent / Documents / Stubs; Other when needed) and does **not** show an under-table Source pages note (smoke redesign).
- **CI maintenance (#116)** — bumped GitHub Actions to current majors that clear Node 20 runtime deprecation warnings (including `upload-pages-artifact@v5` and Node-24 `docker/*` majors); CI `lint-and-test` verifies Python 3.12 only (dropped the 3.13 matrix job) while installs stay `requires-python = ">=3.12"` — 3.13+ issues are accepted but untested in CI; removed the unused `Programming Language :: Python :: 3.13` Trove classifier; deleted non-working Claude Code GitHub Actions workflows; left published `action.yml` on `actions/setup-python@v6` (already node24).
- **`synth --estimate` Candidates is pre-run state, not a run forecast (#113)** — the estimate Candidates block is labelled `Candidates (pre-run state):` with a note that pending sources are not yet reflected. A successful real `synth` prints an end-of-run summary (`Synthesized:`, `Duration:`, and `Tokens:` / `Cost:` when the Claude CLI returns usage via `--output-format json`). Harvest still prints the Candidates report once; the end summary does not repeat it. Docs and Home Commands copy no longer call estimate Candidates a preview of the next harvest.
...
- **BREAKING — page kinds `question` and `comparison` (#109)** — both are gone from the `type:` vocabulary in `llmwiki/schema.py`, which is the single source `frontmatter_validity` and the `wiki_search` MCP tool schema read, so a page declaring either kind is now a lint **error** and `wiki_search` no longer offers them as a filter. Nothing ever wrote a page into `wiki/questions/` or `wiki/comparisons/` — `init` did not scaffold the folders and no synth, harvest, or promote path targeted them — so the two kinds existed only as vocabulary, a graph colour, a legend label, a topic chip, and an Obsidian export target; all of those are removed with them. A vault that carries either folder by hand keeps working: `reindex` catalogues any folder it finds, so the pages stay listed and linked, and `[[wikilinks]]` resolve by filename rather than by folder, so relocating a page into `wiki/concepts/` keeps every inbound link intact without editing the referrers. **Run `llmwiki migrate-page-kinds --vault PATH`** (new in this release, see Added) to do that relocation and the retype for you. The auto-generated `/vs/` model-comparison surface that once shared only the word "comparison" with this kind was later removed in #138.
- **`docs/videos/` recordings (#109)** — `cli-tutorial.{gif,mp4,tape}` and `demo.mp4` are gone. They showed a CLI and a UI several releases behind what the product emits today, and nothing in the repository linked to them. `scripts/record_demo.py` still produces the README's `docs/demo.gif`, writing its intermediate WebM to the gitignored `tmp/demo-recording/`.
- **`/review-pr` slash command** — removed; open-PR review uses `docs/maintainers/REVIEW_CHECKLIST.md` directly (or the delivery-flow local-review stage). Docs and `/maintainer` prompts updated.
- **BREAKING — lint rule `entity_consistency` (#102)** — the rule existed only to demand an `entity_type` value from the fixed seven-value taxonomy, which harvest stamped as `unknown`; every correctly reviewed vault therefore reported one error per promoted entity. The taxonomy is gone with it: `ENTITY_TYPES` / `validate_entity_type()` are removed from `llmwiki/schema.py`, and `frontmatter_validity` no longer inspects the field. Pages that still carry `entity_type` keep it as inert metadata — no migration ships and nothing reads it. `llmwiki lint --rules entity_consistency` now exits non-zero naming the unknown rule instead of silently running zero rules and reporting a clean vault; unknown `--rules` names are validated against the registry generally, not just this one. Do not confuse `entity_type` with `entity_kind`, which is untouched and still drives the AI-model index.
- **BREAKING — CLI flag `synth --allow-unclassified` (#102)** — that flag is gone. #102 made harvest fail closed when a classifier reply was incomplete. **#147 later removed harvest classification entirely**: kinds come from source topic bullets, harvest writes stubs without a model, and a Dummy/`None` backend is enough. Scripts that still pass `--allow-unclassified` must drop the flag. Candidate stubs carry no `entity_type`. The post-write "N filed as `entity_type: unknown`" warning stays deleted.
- **BREAKING — MCP tool `wiki_entity_search` (#102)** — removed outright, with no alias, and folded into a reshaped `wiki_search`. `wiki_search` now takes `term` (required), an optional `kind` (one of `source`, `entity`, `concept`, `project`, `synthesis`, matched against frontmatter `type` — the internal `navigation` and `context` kinds are valid frontmatter but are not offered as a filter, and an unfiltered search still reaches those pages), an optional `format` (`text`, the default prose rendering, or `json` for a machine-readable payload with `pages[].lines[]` and the completeness flags), and the existing optional `include_raw`. `kind` and `include_raw` compose: `include_raw` selects whether `raw/sessions/` is scanned, `kind` filters frontmatter `type` in every corpus scanned. Raw session files carry `type: source`, so `kind=source` with `include_raw` returns matching source pages *and* the raw transcripts behind them; a kind no transcript declares (`kind=project`) just gets nothing from the raw corpus, which is an empty contribution rather than an error. Results are page-level (`path — title` with matching lines indented beneath) rather than bare `file:line`, and pages matching by title or path sort above pages matching only in the body, so a name lookup keeps working. The scan hardening is preserved: one shared hit cap across wiki and raw (#413), and the aggregate byte budget with per-file cap and oversize-skip reporting (#483). The hit cap now stops body-line collection rather than the walk, so a page named for the term is still found and still ranks first when it sits late in the corpus; a matching page cap bounds the output that name matches would otherwise grow without limit. Completeness is reported in two independent fields: `truncated` when an output cap dropped matches, and a new `budget_exhausted` when the scan stopped short of the corpus because the byte budget ran out — previously that case reported `truncated: false` on a partial scan. Agents calling `wiki_entity_search` now get an unknown-tool error; re-read the tool list.
- **BREAKING — `entity_type` in the built search index (#102)** — `search_facets.enrich_entry` stops writing an `entity_type` key onto each entry and `aggregate_facets` stops emitting an `entity_type` bucket, so `site/search-index.json` and its sharded siblings drop the key from every entry and the bucket from `_facets`. Nothing in the shipped site JS reads `_facets`, so the browsable site is unaffected — this breaks only a client reading the index file directly. [`docs/reference/reader-api.md`](docs/reference/reader-api.md) drops the matching numbered invariant from its "anything a client can depend on" list and the surviving invariants renumbered, so a client that cited an invariant by number now reads a different sentence; the list says to cite invariants by the field they constrain instead.

### Fixed

- **A vault-scoped `synth` records its history in that vault (#118)** — `synthesize_new_sessions` derived its `wiki/log.md` target from a module-level path fixed at import time, while taking the pages it writes from the `wiki_sources_dir` it was handed. Since every caller scopes a run by passing `wiki_sources_dir` alone — `llmwiki synth --vault`, `llmwiki add`, `llmwiki all --with-synth`, and the queue runner — a run against one vault wrote its pages there but appended its `synthesize` log entry to whichever wiki sat beside the installed package. The log path now falls back to the wiki directory containing those pages (`<wiki_sources_dir>/../log.md`), the same directory the index rebuild already reconciles; an explicit `log_path` still wins. With no vault in play the two resolve to the same file, so default behaviour is unchanged.
- **`llmwiki init` scaffolds `wiki/projects/` (#102)** — the catalog reconciles a Projects section and the ingest instructions tell agents to file project pages there, but a freshly initialised vault had no such folder. `init` now creates a folder for every page kind the catalog files.
...
- **Theme toggle needed two clicks** — the pre-paint script resolves `data-theme` to a concrete value on every page, but the tri-state cycle read only `localStorage`. With no stored preference the first click re-selected the theme already on screen: state changed, nothing moved. The cycle now starts from the rendered theme and pins its opposite, so every click that changes the theme is visible. Desktop and mobile buttons share one implementation and one label generator instead of two copies that could drift.
- **Graph page swallowed right-click** — the `oncontext` handler called `preventDefault()` before checking whether a node was under the cursor, so the graph was the one page on the site where Firefox mouse-gesture extensions stopped working. The native menu is now only suppressed over a node, where a custom menu replaces it; empty canvas is left to the browser. The scroll `.progress-bar` also stops capturing pointer events.
- **Graph `Cluster` collapsed the whole graph into one node** — clustering keyed on `type`, which is the constant `'topic'` for every node in topic mode, so the control produced a single `topics (N)` dot with no edges. Topics now resolve to the `wiki/` folder behind them (`entities`, `concepts`, `projects`, …) and cluster on that, so collapsing yields `Entities (N)` ↔ `Concepts (N)` with the cross-kind edges aggregated between them. Nodes and the legend are coloured per kind, single-node kinds stay expanded, and the control disables itself with an explanation when a vault has only one kind. Clustering failures report on the page rather than being swallowed.
- **Graph nodes piled up at one point after clustering** — physics is frozen once the initial layout settles, so nodes created by clustering (and released by expanding) had no forces and stacked at the centre. Structural changes now re-run the layout and reframe, reusing the mechanism the layout switcher already had.
- **Wasted vertical space above page content** — the masthead cost ~150px of padding before the first row of content on every page. Hero spacing moved to `--hero-y` / `--hero-gap` tokens so all page families scale together, and section/doc-shell top padding tightened. Home reclaims ~82px above the fold; docs pages open close under the nav instead of 112px below it.
- **Optional-extra hints name the interpreter that is actually running llmwiki** — the missing-converter messages printed a hardcoded `pip install 'llm-wiki[add]'`, which is wrong whenever a bare `pip` belongs to a different Python than the one running llmwiki: a uv-created venv (which ships no `pip` module at all), a wrapper script that execs a venv Python, or a checkout on `PYTHONPATH`. Following the old hint could install the extra into an unrelated interpreter, report success, and leave the import still failing — a silent wrong-interpreter install with no signal that anything went wrong. Hints are now derived from `sys.executable` at call time and pick `uv pip install --python <exe>` / `<exe> -m pip install` / an `ensurepip` fallback to match the environment, installing `-e '<root>[extra]'` when running from a source checkout so the copy being run is the copy that gets the extra. The render-layer hint also stopped naming `llm-notebook`, a distribution that does not exist (the project is `llm-wiki`), and now points `playwright install` at the same interpreter.
...
- **`llmwiki synth` (#90)** — primary command for source synthesis **and** candidate harvest. Default runs both: fill `wiki/sources/`, then write/update stubs under `wiki/candidates/`. `--sources-only` keeps the legacy sources-only path; `--candidates-only` is unchanged. `llmwiki synthesize` remains as a deprecated alias that warns and defaults to `--sources-only` so existing scripts do not suddenly write dozens of stubs. Classification retries omitted names once before fail-closed (exit ≠ 0, write nothing) unless `--allow-unclassified`. `all --with-synth`, `watch`, and status hints call `synth`. `synth --estimate` prints the sources cost estimate plus `Candidates (pre-run state):` (current `wiki/sources/` shape — not a forecast of the next harvest; see #113). Docs / README / `CLAUDE.md` / `AGENTS.md` teach the canonical loop `sync → synth → review → build` (`synth` does not rebuild the site).
- **Home / Analytics candidates review gate (#84)** — Home shows an **Eligible sources** table (Raw → To synthesize → Synthesized; agent chips + `sessions`, Documents as plain text — input counts, see #81) and a **Knowledge layer** table (**Candidates | Entities | Concepts**). Commands include `synthesize --candidates-only`, `candidates list` / promote, and one-shot agent launchers `cd <checkout> && claude|agent|codex "/wiki-candidates"` (Gemini CLI is adapter-scaffold only — no launcher). Analytics shows **Candidates to review** (pending + stale). `llmwiki build` recounts candidates and trusted entity/concept counts into `synth.pipeline` on every build.
- **`llmwiki synthesize --candidates-only` (#90)** — harvests entity/concept **candidates** from already-synthesized `wiki/sources/` into `wiki/candidates/`, closing the gap where synthesis emitted `[[wikilinks]]` that nothing ever materialized (so `candidates list` reported `0 pending` in every vault). Reads the source layer only, never `raw/`: the extraction already happened during synthesis, so the mode runs no per-source synthesis. Cost is at most **one batched call** classifying the harvested names as entity vs concept — it scales with the candidate count, not the corpus. A backend that cannot classify every new target **fails the run and writes nothing** by default — a half-classified queue is not a good queue, and refusing after writing would leave the mess the refusal prevents; `--allow-unclassified` opts into `entity_type: unknown` stubs for review. Harvesting itself never needs a backend, which matters because a vault with nothing left to synthesize is exactly the vault with the largest candidate backlog. `--min-refs N` (default `3`) sets the threshold, counted by **distinct source pages** rather than raw occurrences, so one document naming a target repeatedly still votes once; the default matches the lint rule defining a missing entity page as one "mentioned in 3+ source pages". Stubs land under `wiki/candidates/` only — promotion into the trusted tree stays a human or agent decision via `llmwiki candidates promote|merge|discard`. Re-runs refresh frontmatter and evidence while preserving reviewer prose.
...
- **`llmwiki export` CLI subcommand** — `llmwiki.exporters.export_all` stays; every AI-consumable export (`llms.txt`, `llms-full.txt`, `sitemap.xml`, `rss.xml`, `robots.txt`, `graph.jsonld`, `ai-readme.md`) is written by `build` (it already called `export_all` internally), so there is nothing left for a standalone `export` step to do. Replace `llmwiki export all` with `llmwiki build`.
- **`llmwiki reindex` CLI subcommand** — `llmwiki.reindex.reindex_wiki` / `plan_reindex` stay; catalog reconciliation now always runs automatically inside `sync` and `synthesize` (and already did inside `remove`), so a standalone command is no longer necessary. Run `llmwiki lint --rules index_sync` to verify a vault's catalog matches disk.
...
- **Analytics Wiki value section (#52)** — the Analytics page now leads with usage-led value signals, not just call volume. **Wiki usage** section (merged value + MCP table): retrievals (query/search/read_page) · writes (`wiki_add`) · answer rate · payoff-per-page · distinct attributed caller projects (Tier 1; unattributed calls shown separately). Activity heatmaps — agents sessions/day plus MCP calls/day and optional session/doc read splits from retained `wiki_read_page` telemetry (no daily bar chart). Best-effort wiki-using session days from frontmatter (`mcp__llmwiki__*` tools_used, expanded `CallMcpTool` when origin exists). Sessions vs docs corpus mix and read mix (page counts, no USD split). Top-earning pages and dead stock from `wiki_read_page` paths joined to `wiki/sources` (retained/live telemetry only — not lifetime across compact). Daily MCP series persists in `usage/daily.json` (folded on compact, live overlay refreshed on every build). Synthesis USD stays a muted secondary line when `synth.estimate` exists. CLI `llmwiki usage` unchanged this PR. Tests: `tests/test_usage_daily.py`, `tests/test_wiki_adoption.py`, extended `tests/test_build_analytics.py`.

- **Coding agents are routed to `CONTRIBUTING.md`, and lint runs before every push** — the repo serves two audiences from one tree, and only one of them was addressed. `CLAUDE.md` and `AGENTS.md` are the *product* schema (how an agent maintains a user's `raw/` → `wiki/` → `site/` vault); `CONTRIBUTING.md` governs work on llmwiki itself. Claude Code auto-loads only `CLAUDE.md` and Cursor auto-loads only `AGENTS.md` plus `.cursor/rules/`, and neither root file referenced `CONTRIBUTING.md` at all, so a coding session never saw the contribution rules. Three surfaces now route there: a new path-scoped `.claude/rules/contributing.md` (`paths:` frontmatter, so it loads only when an agent touches `llmwiki/`, `tests/`, `scripts/`, `docs/`, or the root build files, rather than costing context on every vault session), the Cursor rule in `.cursor/rules/read-contributing.mdc`, and `.kiro/steering/contributing-rules.md` — plus a short always-loaded pointer block at the top of `CLAUDE.md` and `AGENTS.md`. `.cursor/rules/` was entirely gitignored, so the Cursor rule existed only on the machine that wrote it; `.gitignore` now allowlists shared rules file-by-file, keeping local rules that name a personal vault path unshared by default. The Kiro steering file had drifted into contradicting this guide (commit types it doesn't accept, a superseded branch name, a hardcoded contributor identity) and is now a thin pointer, since a stale copy of the rules is worse than none. New `.githooks/pre-push` lints the Python files in a push with `ruff` and rejects it on violations — scoped to the pushed diff rather than the whole tree, because a backlog of pre-existing violations would otherwise block every push; it no-ops with a warning when `ruff` isn't installed and honours `LLMWIKI_SKIP_PREPUSH=1`. `setup.sh` / `setup.bat` wire `core.hooksPath`. `CONTRIBUTING.md` gains an **Agent instruction files** section documenting the split and a **Linting** section covering the ruff config, the import-placement convention, and the `# noqa: F401` re-export trap, and its four stale `master` references now say `main` to match the actual default branch.

- **`llmwiki migrate-raw-redaction` (#56)** — deterministic in-place rewrite of `raw/sessions/*.md` so dash-encoded agent-store path segments use the `USER` placeholder (`-Users-<you>-…` → `-Users-USER-…`) without re-converting from `~/.claude/projects` / Cursor stores and without enqueueing `synthesize`. Prefer this over `sync --force` when redaction completeness in already-synced raw matters (agent transcripts are usually retained only ~30 days). Script: `scripts/migrate_raw_encoded_username.py`. Tests: `tests/test_migrate_raw_encoded_username.py`.

- **Session pages surface `cwd` + real `sessionId` + a copyable resume command; project pages are titled by absolute disk path (#36)** — the static site was the browsing UI for session history but gave no way to get back into a session with the originating agent, and no way to tell where on disk a project lives. Session heroes now show the **local** absolute `cwd` (username redaction in `raw/` frontmatter is reversed at build time via `restore_local_path`, so the copyable one-liner is `cd /home/<you>/… && claude --resume <sessionId>`, not the unusable `/home/USER/…` placeholder) plus the full agent `sessionId` (distinct from llmwiki's 8-hex slug). Resume is Claude Code main sessions only (hidden for subagents / non-Claude adapters; greyed out past ~30-day retention). Project pages and `projects/index.html` use the absolute disk path as the **title** (slug stays secondary meta); divergent cwds are listed when they differ. Per-page `.json` siblings, HTML metadata comments, and search-index chunks include `sessionId` / restored `cwd`. Tests in `tests/test_session_resume_cwd.py`.

- **`llmwiki remove <selector>` — cascade removal of raw docs (#42)** — a raw doc fans out into raw file(s), a `synth.files` key, a `synth.pending` backlog entry, and a `wiki/sources` page (plus `--part-NN` pages); deleting the raw file by hand orphans the rest. `remove` plans and executes the whole cascade, then rebuilds the index and appends a `remove` log entry. `--dry-run` prints every affected file, state key and page and changes nothing; a real run requires `--yes` or an interactive confirm, holds the pipeline lock, and re-plans under it. The selector globs the project dir and the rel-path (with or without `.md`) — **not** the bare file stem, which would cross project boundaries. A page whose `source_file` names a different raw file is never deleted, and if any raw doc cannot be unlinked the run aborts (exit 2) before touching derived pages or state rather than leaving the vault half-removed.

- **MCP usage + raw-docs on the Analytics pages (#27)** — the static site now answers "is this wiki earning its keep, and who actually uses it?" without leaving the Analytics page. The page opens with a single row of stat cards — **Tokens** (total + per-session avg, merged from the old "Total tokens" and "Average per session"), **Best cache hit**, **Heaviest project (by tokens)**, and **Heaviest project by MCP usage** — followed by a **Wiki usage (MCP)** section: a one-line totals caption (calls · items returned · server processes · raw documents) plus a per-tool table of **calls**, **items returned**, and **zero-hit rate**. Items returned counts only entity-returning tools (`query`/`search`/`list_sources`/`read_page`/`entity_search`/`category_browse`/`export`/`confidence`); `lint`/`sync`/`lifecycle`/`dashboard` show `—`. Response bytes are collected but no longer surfaced — a low-signal metric. The distinct-client metric is reported honestly as **MCP server processes** (distinct `(server_pid, server_started)` pairs), never as "sessions". Per-**project** pages gain a matching stats block — raw-doc count, MCP calls, items returned, server processes — plus a per-tool calls/answers table. The activity heatmap is renamed **Agents Activity** (it covers agent sessions, not raw docs) on both the global and per-project pages. `synthesize` now writes a producer breakdown into its log entry (e.g. `Processed: 2 Claude · 1 Cursor · 3 docs`), which **Recent activity** renders verbatim. Aggregation reuses `usage.combined_totals`, read from the vault content root, so numbers are as-of-last-build; a legacy `rollup.json` lacking the new fields reads as zero.

- **MCP usage telemetry (#26)** — the MCP server now logs every tool call locally so "is this wiki earning its synthesis spend?" is answerable. Each call appends one JSON record (`tool`, `query`, `hits`, `resp_bytes`, `duration_ms`, `caller_project`, `server_pid`, `server_started`) to a **per-process** file under `<vault>/usage/` (`mcp-<pid>-<start>.jsonl`), merged at read time — zero write contention across the several concurrent server processes a real setup runs, and telemetry never touches `llmwiki-state.json`. `hits == 0` (a knowledge gap or noise) is recorded distinctly from an unknown count. Writes are strictly best-effort: a telemetry failure never breaks a tool call. Opt out with `LLMWIKI_MCP_TELEMETRY=0`. New `llmwiki usage [--json] [--compact]` folds the logs into totals (calls / zero-hit rate / bytes per tool and per project) and prints them next to the synthesis cost already persisted in `synth.estimate`. `--compact` rolls whole past months into a kept-forever `usage/rollup.json` and deletes the raw files, so aggregation stays cheap while lifetime totals survive. Scope is MCP calls only — `file://` static-site browsing stays untracked; on-site rendering is deferred to #27.

- **`filters.include_subagents` mode (#30)** — controls what happens to subagent transcripts (`is_subagent: true`), which dominate a real corpus (~59% of raw sessions) and are mostly redundant with their parent session's synthesis. Three modes: `all` (sync AND synthesize like any session), `only-raw` (**default** — sync into `raw/` but skip in `synthesize`/`queue run`, and don't count toward the `queue status` `unsynth_total` backlog), and `off` (don't convert them at all). `only-raw` is a policy, not a staleness state: `synthesize --force` re-does synthesis but does **not** resurrect subagents — switch to `all` to make existing raw subagents eligible (they then appear as backlog). `off` only affects future syncs; it never deletes existing `raw/` files.
...
- **`claude` synthesis backend strips agent scaffolding from every call (~9x cheaper per page)** — synthesis is text-in / text-out: the prompt carries the format rules, the topic vocabulary, and the session body, and the backend only ever reads stdout. But a bare `claude -p -` wraps each call in a full coding-agent context — its system prompt, every built-in tool schema, every configured MCP server's tools, auto-discovered `CLAUDE.md` files, and the skill listing — none of which a synthesis call can reach, and all of which are billed as input on every page. Measured with a trivial prompt in a repo with several MCP servers configured: **35,081 input tokens without the new flags vs 700 with them**, i.e. ~95% of the input bill was scaffolding. Calls now pass `--tools ""`, `--strict-mcp-config`, `--disable-slash-commands`, `--setting-sources ""`, and a short `--system-prompt` in place of the agent's. End-to-end on one real page with `--model sonnet`: **$0.369 → $0.042 per page** (58,169 → 4,607 input tokens; output unchanged in shape, 1,299 → 730). Prompt, output contract, and page format are untouched — the first line is still the `<!-- suggested-tags: … -->` comment and all sections render as before. `--tools` is variadic, so its `""` must be followed by another `--flag`; the argv order is pinned by a test. Opt out with `"synthesis": {"claude_lean": false}` (only an explicit `false` opts out — a missing or malformed value keeps the default on), which restores the previous behaviour for anyone who deliberately wants `CLAUDE.md` context during synthesis. The default `claude_model` stays `sonnet`: Haiku matches it on Summary / Key Claims but degrades on `## Connections`, linking incidental libraries and coining lowercase-with-spaces pages instead of the project entity — and `## Connections` is what feeds `llmwiki graph`, the backlink index, and the topic vocabulary injected back into every later prompt. New docs: [`docs/reference/synthesis-cost.md`](docs/reference/synthesis-cost.md), covering how LLM per-MTok pricing works, where the money goes, the Sonnet-vs-Haiku measurements (including Haiku's default extended thinking, billed as output at ~5x input), and how `synthesize --estimate` prices a run. Tests: 6 new cases in `tests/test_synth_claude_cli.py`.

- **Synthesis prompts split into a cached prefix + a per-page tail (~25% cheaper again)** — input to the `claude` backend is billed as a 1-hour cache *write* at 2x the fresh rate. Measured across 29 pages, `cache_read_input_tokens` was **0** on every one: the CLI puts a single cache breakpoint at the end of the user message, so the key covers the page body and no two pages ever share one. The format rules and the injected topic vocabulary — byte-identical for every page of a run — were being re-written at 2x each time and never read back. `llmwiki/synth/base.py` now exposes `split_prompt_template()`, which cuts the template at its `## Session to synthesize` marker into a run-stable prefix and a per-page tail. This is a **provider-agnostic** contract, not a Claude workaround: every provider bills a repeated prefix more cheaply, they just want it in different places, so each backend maps the stable half onto its own mechanism — the `claude` backend passes it as `--system-prompt` (the only slot the CLI reuses across invocations), and the `ollama` backend passes it as the `system` field on `/api/generate` (no billing, but it keeps the KV-cache prefix stable across pages). A future OpenAI/OpenRouter backend puts it in a leading system message for automatic prefix caching; the scaffolded Anthropic API path already has `cache_control` for the same purpose. A custom template with no marker yields an empty prefix and the whole template as the tail, so it still synthesizes correctly — caching is an optimisation, never a correctness requirement. Measured on real pages: 4,642 tokens read back per page after the first, `$0.076 → $0.057`/page steady state. Output format and `Connections` quality verified unchanged on real sessions.

- **Repeated `[[links]]` are dropped from `## Connections` at write time** — models sometimes list the same scope in two bullets of one page (`[[MCP]]` twice), more often on the smaller ones. `_dedupe_connections` keeps the first bullet, which carries the description the model wrote first. Scoped to the Connections section: a scope named in `## Summary` and again under Connections is ordinary prose, not a repeat. Matching is case-sensitive, so `[[MCP]]` and `[[Mcp]]` both survive — they are distinct nodes to the graph, and collapsing spelling variants belongs to the topic-consolidation pass. Aliased links (`[[Target|alias]]`) de-dupe on the target, using the same `WIKILINK_RE` the graph builder uses so "a link" means one thing on both sides. This is a rendering fix, not a correctness one: `graph.py` already collects links into a set and topic edge weights count distinct sessions, so duplicates never inflated the graph — they were just visible on the page.

- **`synthesis.claude_effort` exposes `--effort`** — extended thinking is billed as output at ~5x the input rate, so on the small models it dominates the bill: Haiku spent **5,753 output tokens** on one page at its default effort versus **1,609** at `low`, for a summarization task whose reasoning is already spelled out in the prompt. Unset by default (no flag passed, current behaviour preserved); set `"claude_effort": "low"` when running Haiku, or the model's thinking cancels out its lower rate.

- **`_VOCAB_LIMIT` raised 80 → 200** — the topic vocabulary now rides in the cached prefix, so it is written once per run and read at 0.1x on every later page. Breadth stopped being a per-page cost. A topic missing from the list gets re-coined under a new spelling, fragmenting the graph and the backlink index — the exact failure the list exists to prevent, previously traded away to save tokens that caching now makes cheap.
...
- **The `claude` backend no longer inherits Ollama's `timeout` (#65 follow-up)** — `synthesis.timeout` was read by *both* backends, and the shipped `examples/sessions_config.json` sets it to `60` for Ollama. Any user on the `claude` backend therefore ran with a 60s per-page cap instead of the documented 180s default, on a setting they never aimed at it — real pages take 19-37s, so a long transcript failed on a value meant for a different backend. The `claude` backend now reads `synthesis.claude_timeout` (default 180) and never the shared key. Related: `docs/configuration-reference.md` has always documented Ollama's settings as the nested `synthesis.ollama.*` block, but `load_ollama_config` only ever read them flat off `synthesis.*` — which is what created the collision. The nested block is now read and takes precedence, with the flat keys kept as a working fallback so existing configs are unaffected, and `examples/sessions_config.json` moves its Ollama keys into the block.

- **`synthesize --estimate` prices what the `claude` backend actually sends** — the estimator predated this backend and modelled an Anthropic API-style call: a stable prefix (`CLAUDE.md` + `wiki/index.md` + `wiki/overview.md`) written to cache once and re-read on every later page. The CLI backend does none of that. Four independent errors, each verified against `claude --output-format json`: (1) **wrong prefix** — ~32k tokens of files that are never sent were billed on every page, while the scaffolding and prompt template that *are* sent went uncounted; `prefix_tokens` now means per-call overhead + the rendered prompt template, and the template is measured with its injected topic vocabulary rather than assumed fixed, since the vocabulary grows with the wiki and is re-sent every call. (2) **wrong cache model** — assumed 1 cache write + N−1 hits, but each page is a separate process, so cost is linear in page count. (3) **wrong token ratio** — `estimate_tokens` used 4 chars/token (English prose) and its docstring claimed it *over*-counts code; measured against the real prompt, 8,567 chars → 3,677 tokens, i.e. 2.33 chars/token, a ~1.7x **under**-count. Adds `TRANSCRIPT_CHARS_PER_TOKEN` and an optional parameter; the prose default and every existing caller are unchanged. (4) **wrong body size** — bodies were counted in full though `claude_cli.py` truncates to 8,000 chars, and a multi-part doc was billed as one call instead of one per chunk. The report now also splits the fixed cost (`overhead_tokens` / `template_tokens`) so a surprising figure is traceable to either the agent scaffolding or a bloated vocabulary, and warns when `claude_lean` is off. Net on a real corpus: `$4.76 → $7.61` for the same backlog, against `$0.042`/page measured end-to-end (modelled `$0.038`, ~10% under). Tests that encoded the old model are replaced rather than deleted — the prefix-too-small warning is gone (a small prefix is now the goal, not a problem), and new cases pin that `CLAUDE.md`/`index.md`/`overview.md` no longer move the estimate, that over-cap bodies are not billed, and that non-lean costs multiples more.

- **Rate card: `sonnet-5` was priced 33% low** — listed at `$2/$10` per MTok with a stale "intro through 2026-08-31" note. Derived from `modelUsage.costUSD` (905 input + 494 output = `$0.010125`), the real rates are `$3/$15`. `haiku-4.5` (`$1/$5`) verified correct by the same method.

- **Site-overview synthesis no longer hardcodes a model** — `build.py` pinned `claude-haiku-4-5-20251001` inline, so it could not be changed and did not track the rate card. It now reads `synthesis.overview_model` (default `haiku`) and builds its command line through the same `lean_argv()` helper as page synthesis, so the scaffolding-stripping flags cannot drift between the two `claude` call sites.

- **Two stale tests unrelated to synthesis** — `test_render_index_is_tree_browser` still asserted pre-Home-widget markup (`Queue dashboard`, plus a `Commands` `<summary>` that `render/js.py` now renders client-side), and `css.py` had outgrown its line ceiling after the Home pipeline State table + shared collapse-section chrome landed. Both updated to current reality; the suite is green.
...
- **Index pages restore cwd paths consistently; encoded path segments covered by redaction (#56)** — follow-up to #36/#55. Three holes: (1) copying the examples `"real_username": ""` placeholder into root `config.json` wiped the autodetected username on overlay, so `restore_local_path` became a no-op and `projects/index.html` mixed `/Users/USER/…` with never-redacted real paths; `_ensure_real_username` now re-runs after overlay. (2) `sessions/index.html` never restored paths — it now has a **Cwd** column via `local_cwd` and restores paths inside description text. (3) redaction only rewrote the leading `/Users/<you>/` (or `/home/<you>/`), leaving the real name in dash-encoded agent store segments (`~/.claude/projects/-Users-<you>-…`); both redact and restore now cover `-Users-` / `-home-` encoded prefixes via shared `_substitute_path_username`. Tests in `test_username_autodetect.py`, `test_convert.py`, `test_session_resume_cwd.py`.
  **Agent / vault action (required for site correctness):** after upgrading the engine, rebuild **the user's vault** (not the llm-wiki clone): `llmwiki build --vault <vault>`. Skipping rebuild leaves a stale `site/` — mixed `USER`/real project titles, no sessions Cwd column, descriptions still showing `/…/USER/…`, while session detail pages (if rebuilt earlier under #55) already show restored resume paths. See [`docs/UPGRADING.md`](docs/UPGRADING.md) § “v1.5.0 — index cwd restore…”.
  **Optional raw rewrite (deterministic, no LLM):** if the user publishes/shares `raw/` and wants the `USER` placeholder complete in already-synced files, run `llmwiki migrate-raw-redaction --vault <vault>` (script: `scripts/migrate_raw_encoded_username.py`). It rewrites path strings in `raw/sessions/` only — no `synthesize`, no wiki churn. **Do not** `sync --force` / re-convert from `~/.claude/projects` or Cursor folders for this: agent transcripts are usually kept only ~30 days, so older `raw/` rows often have no source left; agents that follow force-sync with re-synth also burn tokens for no benefit. Private local vaults: rebuild alone is enough.

- **Sessions table sticky header no longer overlaps the first data row** — `.table-wrap { overflow-x: auto }` made the wrap the sticky containing block, so `thead { top: 56px }` (nav clearance) shoved the header into the first `<tbody>` row on initial paint and broke page-scroll stickiness. Default wrap no longer sets overflow; sticky keeps `top: 56px` against the viewport. Narrow viewports (`max-width: 900px`) still get `overflow-x: auto` with `top: 0` so columns can pan without reintroducing the overlap.

- **Sessions table Date column shows the full `YYYY-MM-DD`** — the Date `<col>` was 10% under `table-layout: fixed` with ellipsis on every cell, so dates rendered as `2026-07-…`. Date keeps a dedicated width (now alongside the #56 Cwd column), table `min-width` is 1100px, and the Date cell opts out of ellipsis.

- **MCP telemetry attributes each call to the project that made it (#51)** — every record was stamped with the *server process's* `os.getcwd()`, captured once in `UsageRecorder.__init__`, on the assumption that a stdio server inherits the caller's working directory. It doesn't: a server launched out of `~/code/llm-wiki` reported `llm-wiki` for calls that came from every other project, and one process serving many sessions over its lifetime cannot describe them with a single value anyway — so the per-project breakdown (#26/#27) and everything built on it were wrong. On a real vault the cwd basename also produced buckets that are not projects at all (a `scripts/` subdirectory the user happened to be in). Attribution is now resolved **per call** from a caller-supplied signal, recorded alongside a new `caller_source` field, best signal first: (1) `CLAUDE_PROJECT_DIR` — the workspace path Claude Code auto-injects into every stdio MCP server (≥ v2.1.139), zero config, and stable per-caller because Claude Code spawns one server per session; (2) the client's own workspace directory via an MCP `roots/list` request (asked only of clients that advertised the `roots` capability, and only after `notifications/initialized`, so clients that read one response per request are undisturbed); (3) a path argument carrying the caller's cwd encoded into a single segment (`…/-home-dev-code-my-app/…`, as agent scratchpads and session stores write it); else honestly `unknown`. Serving roots required the stdio loop to accept inbound JSON-RPC *responses* — a frame with an `id` and no `method` used to draw a bogus `-32601` back. Paths *inside* the wiki (`wiki/sources/my-app/…`) are deliberately not used: they name the subject of a call, not its caller, and attributing on subject would make every retrieval look same-project and erase the cross-project signal this telemetry exists to measure. The server's own `os.getcwd()` is never used either — a client may launch the server anywhere (Claude Code's desktop app uses `$HOME`), so it is unrelated to the caller. Unattributed calls still count toward totals but are never presented as a project — they print as `(unattributed)` in `llmwiki usage` and are excluded from the site's "Heaviest project by MCP usage" card, which no longer risks linking to a project page that doesn't exist. Records and rollups written before this change carry no `caller_source` and are read as unattributed rather than under the server's directory name. All three project signals feed one shared `slugs.project_slug_from_abs_path`, reconciled with the Claude Code session adapter, so a project resolves to the same slug — matching its own page — whether it arrived through live telemetry or through ingestion. Coverage: Claude Code attributes every call with no setup; Cursor advertises `roots` but errors on the call and injects no workspace env, so its calls stay `unknown` (or use a path arg where present) until it ships a fix. 39 new tests: `tests/test_mcp_caller_attribution.py` (28) plus cases in `test_slugs.py`, `test_mcp_protocol.py` (end-to-end roots), `test_build_analytics.py` and `test_usage_cli.py`.

- **Search works when the site is opened over `file://` (#20)** — the built site is fully static, but double-clicking `index.html` produced a search palette that opened, accepted typing, and returned nothing for every query. Browsers block `fetch`/XHR against `file://` URLs, so `search-index.json` and every `search-chunks/*.json` failed to load; the client's `.catch(function () { idx = []; })` then turned each CORS failure into an empty index, making a completely broken page indistinguishable from a corpus with no matches. Subresource *execution* is not restricted the same way, so `build_search_index` now emits each payload a second time as a `.js` sidecar (`write_js_sidecar()` in the new `llmwiki/render/data.py`) that assigns itself into `window.llmwikiData`, keyed — for chunks — by the exact manifest path the client already holds, so there are no key-derivation rules to keep in sync. The client's shared loader `window.__llmwikiLoadData(url, key)` injects a `<script>` tag instead of fetching, and all four former fetch sites (palette index, per-project chunks, wikilink-preview fallback, related-pages fallback) route through it, so one code path serves `http://` and `file://` identically. U+2028/U+2029 are escaped in sidecars (legal in JSON strings, illegal in JS string literals before ES2019). The `.json` originals stay on disk — they are the documented reader-API surface external tools consume — and the inert `<script src="search-index.json" type="application/json">` hint tag, which never executed or exposed anything, is dropped in favour of a `window.LLMWIKI_INDEX_JS_URL` global. Measured on a real corpus, same page, protocol the only difference: `file://` went from 0 to 801 loaded entries. The graph half of #20 needed no change — `graph.html` has embedded its payload at build time since #118. 16 tests (`tests/test_file_protocol_search.py`).

- **Search actually opens on `graph.html` (#20)** — the graph page rendered the nav's search button and loaded `script.js` specifically so the command palette would work there (#456), but the palette *dialog* markup lives in `page_foot()`, which `graph.html` never called — it injected only `nav_bar()`. So `#palette` was absent, `openPalette()` returned early, and both the button and <kbd>Cmd</kbd>+<kbd>K</kbd> silently did nothing; the page also never set `LLMWIKI_INDEX_URL`. Broken on every protocol, not just `file://`. The palette + shortcuts dialog + search-data globals are now a shared `search_palette_markup(js_prefix)` helper used by both `page_foot()` and the graph template, so the two can't drift, with a test asserting `script.js` is still loaded exactly once (two tags would double-bind every listener).

- **Static-site failures are visible instead of silent (#20)** — new `window.__llmwikiReportError(context, err)` writes to `console.error` *and* renders a dismissible `role="alert"` bar on the page, de-duplicated by message and styled inline so it still renders when the stylesheet is what failed. Every search load path reports through it, and the palette now distinguishes the two states an empty result list used to conflate: a `.palette-note` row reads "Search data could not be loaded — this page is broken, not empty." when the index is unreachable, or "No matches, but part of the search data failed to load." when a single chunk is missing (one bad chunk degrades search rather than aborting it). CONTRIBUTING gains a **Static-site error handling** section banning silent `catch` handlers in emitted JS and a matching TL;DR rule.

- **`filters.exclude_headless` only worked at ingest, so headless sessions already in `raw/` were synthesized forever (#8 follow-up)** — synthesis shells out to an agent CLI; that call is itself logged as a session, `sync` ingests it, and the next pass pays to summarize the wiki's own output, producing more headless sessions in turn. The filter blocked *new* ones but nothing excluded those converted before it shipped (or while it was off), so they sat in the backlog permanently. The setting now governs the backlog too: a headless session in `raw/` is never synthesized (even under `--force`) and never counted as pending — the same contract `include_subagents: only-raw` already had for subagents. `raw/` is untouched; this is an eligibility policy, not a retention one.

- **Sessions carried no record of how they were launched** — the converter read `entrypoint`/`promptSource` to filter at ingest but never wrote them, so once a session was in `raw/` nothing could tell an automated `claude -p` run from a real one, and the classification could not be revisited without the original transcript. Session frontmatter now records `entrypoint`, `promptSource`, and a resolved `is_headless` flag. Files converted before this read as "not headless" rather than being reclassified by guesswork — re-sync to classify them.

- **`synthesize --estimate` quoted work that `synthesize` would not do** — the estimate applied the subagent policy but not the headless one, and `synthesize_new_sessions` resolved its backlog through a helper that ignored the caller's policy. The two now resolve eligibility identically and from the same config key, and the estimate prints what it excluded (`Not eligible (synthesize skips these too): …`) instead of silently dropping it.

- **`llmwiki add` silently dropped every paragraph containing an inline link** — at trafilatura's default recall, a paragraph whose text carries an `<a>` is frequently pruned as boilerplate, so `include_links=True` preserved citations only in the paragraphs that happened to survive. Measured against a CMS's own REST API as ground truth over a 9-page sample, default extraction lost up to 16% of the article — and what it lost was specifically the cross-references ("see part 2", source citations) that synthesis links on. `favor_recall=True` matches ground truth and pulled back no nav/header/footer on any sampled page, so extraction now uses it. The kwargs test asserts `favor_recall` is set and `favor_precision` is not, pinning both failure modes.

- **Body subsections were used as document titles** — `derive_title` took the first markdown heading of any level. A CMS article's `<h1>` frequently sits outside the container an extractor treats as content, so that first heading is an `##` body subsection. Whole families of articles collapsed onto the same generic title — every bank's guide named after its "signing in" section, colliding into `-2` directory suffixes — while the correct, specific title sat unused in `html_title`. Only an H1 now counts as the document's own heading; anything else falls through to the cleaned HTML `<title>`. `first_heading()` gained a `levels` argument for this, and a lower-level heading is still the last resort for documents with nothing else to offer.

- **Chunk sub-titles kept their markup** — `add_doc._first_heading_line` was a near-duplicate of `slugs.first_heading` that did not strip inline markup, so an oversized document's `(part N/M: <sub>)` title suffix still carried the permalink anchor even after the document title was cleaned. It now delegates to the shared helper, removing the duplication along with the bug.

- **Heading permalink anchors leaked into titles, slugs and directory names** — CMS themes render a self-link inside every heading (`<a href="#0-toc-title">#</a>`). Recall-mode extraction preserves it, so `first_heading` returned `Refund policy? [#](#0-toc-title)` and that markup propagated into the page title, the slug, and the `raw/docs/<project>/` directory name. Heading text is now reduced to plain prose: punctuation-only link labels (permalink glyphs) are dropped whole, real link labels are kept without their target, and emphasis/code markers are stripped.

- **Two extraction tests passed only where trafilatura was uninstalled** — they asserted the stdlib fallback path but forced it by deleting the `sys.modules` entry, which merely clears the import cache; with the package actually installed the real module was re-imported and the assertions flipped. They now insert a `None` sentinel, so the fallback is exercised either way.

- **`llmwiki add` extracted nothing from real article pages (#42 regression)** — the `favor_precision=True` added for boilerplate removal was too aggressive: on real-world article pages trafilatura discarded the entire body as low-confidence and returned only a skip-to-content link (~33 chars). Dropped it — trafilatura's default recall already strips site chrome while keeping the article body (verified end-to-end on a live documentation page: 33 chars of nav becomes ~1.9k chars of clean content, no navigation). The kwargs test now asserts `favor_precision` is unset, so the regression can't return.

- **Topic pages linked documents to `sessions/` and 404'd (#45)** — `wiki-add` raw docs compile to `documents/<project>/<stem>.html`, but the graph's site-URL mapper always emitted a `sessions/…` path for source pages with no `source_file`, so every topic-page and graph link into the whole `documents/` layer was dead. A `source_file: raw/docs/…` was worse: it fell through the `raw/sessions/` split and produced no link at all. The mapper now picks the prefix from the page's `raw-doc`/`wiki-add` tags (and honours a `raw/docs/` `source_file`), leaving genuine session pages on `sessions/`. Dead URLs are still nulled against the built site, so an unresolvable page degrades to a disabled click rather than a 404.

- **URLs with no reachable content are reported, not landed** — a page that returns HTTP 200 but yields no article body (a stale/renamed URL serving the site's shell, or a page that renders its body client-side) used to be written as a navigation-only raw doc: worthless in the wiki and still costing synthesis tokens. `convert_url` now flags it (`ConvertedDoc.no_content`) and `add_sources` skips the write and reports the URL in `errors`, so a batch add returns a list of unreachable sources instead of quietly filling `raw/docs/` with navigation. The signal is the shell signature — substantial markup with no extracted text — so a genuinely short page (small markup) still lands normally.

- **URL adds kept site boilerplate (#42)** — `llmwiki add <url>` now drives trafilatura with `favor_precision`, `include_links` (prev/next-part links and citations survive), `include_comments=False` and `include_tables`, and records `extractor: trafilatura|stdlib` in the raw-doc frontmatter so stdlib-produced docs stay identifiable. Previously a page whose navigation is `<div>` soup rather than semantic `<nav>`/`<header>`/`<footer>` came through with all its chrome, which also inflated pages past the chunk cap and split one article into navigation-only part files. When trafilatura is missing on an HTML source the warning now says so explicitly and points at `pip install 'llm-wiki[add]'`.

- **Commands acted on the git clone instead of the configured vault (#43)** — `REPO_ROOT` is the code checkout, but `lint`, `export`, `graph`, `candidates` and `serve` took their directory defaults from it, so with `vault.default_path` set they read and wrote the repo's seed demo content. `lint` was the sharpest case: it accepted `--vault` but applied it only to the state file, never to the pages it linted. All now resolve through one `_content_root()` chokepoint, and `graph` / `export` / `candidates` gain a `--vault` flag (`serve` resolves from config only — `serve --vault` stays removed — and an explicit `--dir` still wins). `llmwiki all --vault X` now propagates the vault into its graph / export / lint steps and into the `--strict` lint re-read, which previously graphed, exported and gated on the clone; `export` also remaps `all`'s always-truthy `--out` default the way `build` does. A configured-but-unusable vault is now a hard error (exit 2), matching `build` / `all`, instead of silently falling back to writing inside the git clone on a typo. `graphify_bridge` is parameterized (`wiki_dir` / `graph_dir` / `graphify_out`) so the default graph engine keeps working for vault users instead of being skipped, and its output paths no longer raise `ValueError` when they sit outside the repo.

- **Knowledge-graph layout is readable instead of a dense clump (#21)** — the viewer now uses vis-network's `forceAtlas2Based` solver (which spreads a hub-heavy topic graph far more evenly than the old `barnesHut`, which let the dominant hub collapse everything inward), runs a real 2000-iteration stabilization instead of 200, and draws edges with the static `cubicBezier` curve type so curvature survives the physics freeze (`continuous` rendered near-straight lines). A new **Layout** dropdown beside the cluster toggle switches density live — `sparse` (default) for a roomy spread, `tight` to reel single-edge leaf topics back toward the cluster — with the choice persisted in `localStorage`. The #9 freeze contract is preserved: the graph is stable from first paint and after every switch (each layout change kicks an explicit `network.stabilize()` because `setOptions` alone re-enables live physics without ever firing `stabilizationIterationsDone`, which would otherwise leave the new layout shaking forever).

- **`sync` now honors the `filters` in your root `config.json` (#25)** — `convert_all` (used by `sync` and the queued session-sync) resolved its config from `examples/sessions_config.json` **only**, so a `filters.exclude_temp_cwd: true` (or any other convert-side filter) set in the gitignored root `config.json` silently had no effect on the one command it exists for — even though the synth/schedule paths already honored it. `convert_all` now merges `config.json` section-wise over the shipped example (matching `config_schedule._load_sessions_config`) whenever it is called without an explicit `config_file`; an explicit path stays a single-file read. The shipped example's stale "loaded when no ./config.json exists" note is corrected to describe the merge.
