...
- **`candidates discard --redirect PAGE` and `migrate discarded-topic-links` (#282)** — `discard` now rewrites every `[[link]]` to the discarded name outside `wiki/archive/` (any casing, with or without label/anchor) to plain text, keeping the visible text; with `--redirect PAGE` (CLI, `apply --actions` `redirect` field, and a *Redirect links to* field on `candidates.html`) the links become `[[PAGE|text]]` and the name is recorded under the page's `## Aliases`, so later links resolve there and the synth vocabulary maps the name onto that page. `reason.txt` gains `Redirected to:` and a correct `Original path:`. New offline `llmwiki migrate discarded-topic-links --vault <vault> [--dry-run] [--redirect NAME=PAGE …]` applies the same rewrite to candidates discarded before this release and moves slash-nested stubs to their flat path; zero LLM, idempotent. `discard()` returns a `DiscardResult` (archived `path`, rewrite counts) instead of a bare path. Every `candidates` slug (`--slug` on `discard`, `promote`, `flip-promote`, `merge`, `rewrite-key-facts`, plus `merge --into` and `discard --redirect`) now resolves through one shared resolver: the exact filename first, then a unique `norm_page_key` fold of the filenames that action searches — the pending stubs for the review actions, the trusted `entities/` and `concepts/` pages for `rewrite-key-facts` — so a differently-cased or differently-punctuated slug still finds a page such as `JUNK.md`; a fold matching more than one raises naming them all instead of guessing. The shared resolver also sanitizes the slug through `candidate_filename` and refuses a resolved path outside the vault, so `--slug ../../outside` names nothing instead of reading and rewriting a file next to `wiki/`. A redirect whose name another live page already answers to is refused before the stub moves, naming that page, so two live pages never claim one name; a `--redirect` combined with any action but `discard` is refused by the CLI, the pasteable commands, and batch validation instead of being dropped; every page a rewrite could not read is named (`discard` and `apply` print a warning, the migration reports it and exits non-zero) instead of being skipped in silence; `migrate discarded-topic-links` validates every `--redirect` pair before the first write, so a bad one leaves the vault untouched, and reports a pair whose name a live page already answers to as skipped rather than dropping it; the redirect target keeps its own mention of the name as plain text instead of linking to itself; an `## Aliases` entry round-trips a name that contains an em dash, so a re-run appends nothing; and a batch of discards now costs one live-page scan plus one rewrite pass instead of two passes per row, with per-row counts unchanged. `migrate discarded-topic-links` also refuses to flatten a name a reviewer **merged**: an archived candidate whose `reason.txt` records `merged into <target>` but that no live page answers to keeps its links and is reported under `merged, left linked:` with a ready-to-paste `--redirect "<name>=<page>"` line (the page answering to the recorded target, else the single live page whose `norm_page_key` contains or is contained by it), exiting 1 so a script notices; `--force` unlinks those names like any dismissal, `--dry-run` shows the same partition, and the run's other work still applies. A merge made before survivors recorded `## Aliases` (#139), or one whose survivor was later renamed or re-filed, leaves that reason file as the only record of where the links belong — the new `candidates.merged_intents(wiki_dir)` reads it back through the same formatter `merge` writes it with.
  - *Release note:* Discarding a candidate now removes or redirects the links to it; run `llmwiki migrate discarded-topic-links` once to clean up older discards (#282).

- **Curated knowledge reaches the browsing reader (#248)** — every `wiki/entities/` and `wiki/concepts/` page now gets a topic page and a `⌘K` entry however few sessions cite it (`build_topic_graph(..., include_curated_pages=True)`, passed by the site build only, so candidate harvest keeps its own vocabulary); `wiki/archive/` and `_`-prefixed folder-context stubs stay out. Topic pages are written even when the graph is too sparse for the viewer — `graph.html` still falls back to the page graph below `_TOPIC_GRAPH_MIN_NODES`, but a small vault no longer loses its curated pages. One exception, approved after review of the built site: a topic with **no connected topics and no content of its own** gets no page at all — the page would show a title, `No connected topics.` and nothing else. Both conditions must hold, so a reviewed page nobody co-cites and a well-connected page recording no facts both keep their page. The skip is computed once over the node list, so such a topic has no page, no `⌘K` entry, no row or count on `topics/index.html` and no graph node, while its wiki page stays in the Wiki search group with `url: null` and an unclickable row. New **Topics** nav entry (desktop row and mobile drawer) pointing at `topics/index.html`, which now renders three counted sections — Entities, Concepts, Other topics — with a kind chip on curated rows and today's reach order kept within each. `⌘K` returns two always-visible groups, Wiki then Site: the Wiki group mirrors `wiki_search` `mode=match` over a new lazy `search-wiki-corpus.json` payload (optional `_wiki_corpus` manifest key, `.js` sidecar for `file://`) carrying the same deterministic, containment-checked wiki walk as assistant search, including its 4 MiB per-page and 50 MiB aggregate input caps. Literal substring matching puts name matches before body-only matches, both path-sorted, with 200-page / 200-line output caps — replacing the palette's fuzzy scoring for wiki results, so multi-word queries that are not literal substrings return nothing exactly as the assistant does. Input-cap status is carried in `_wiki_corpus_status`, and an incomplete static corpus warns instead of making an absolute no-match claim. Every occurrence of the searched term is marked in a result's title, path and matching lines in both groups, with a display window that keeps a late-in-line hit on screen, so a row shows why it matched. When a wiki summary and a raw session or document match and open the same reader URL, the Wiki row is shown once instead of repeating the destination in Site. A wiki page with no reader page is listed but not clickable and is skipped by keyboard navigation; a missing or failed payload reports on the page instead of rendering an empty group. `type:` / `project:` / `model:` / `date:` / `tags:` / `sort:` filter the Site group only; `kind:` applies to both. Empty Site input remains a 10-row preview; any explicit Site text/filter/sort query may return up to 200 rows, with exact `Showing 200 of N` feedback for capped filter-only and sort-only results. Rebuild the site to pick this up.
  - *Release note:* Every curated entity and concept that records something or is co-cited gets a site page, a **Topics** tab lists them by kind, and `⌘K` searches wiki page text with the same rule your assistant uses, highlighting what you typed in every result (#248).
...
- **Discarded candidates stay out of the synth vocabulary; slash names stay flat; case variants count together in lint (#282)** — the topic vocabulary (`derive_vocabulary` → `build_topic_graph` → synth prompt) drops every name discarded under `wiki/archive/candidates/` in any casing, via the shared `candidates.discarded_names`; harvest also reads archived stub titles, so a discarded `A/B thing` is not re-proposed. Candidate stubs are written to a flat, path-safe filename (`candidates.candidate_filename`: `A/B thing` → `A-B thing.md`, title kept verbatim) instead of a subfolder; the same helper drops trailing dots and spaces and suffixes `-` on a name Windows reserves for a device (`CON` → `CON-.md`), so a stub is openable on every platform. `link_integrity` folds `[[Foo]]` / `[[foo]]` references by `norm_page_key` before the `min_refs` threshold, and `## Aliases` lookup is case/punctuation-insensitive. `norm_page_key` keeps non-Latin letters, so distinct Cyrillic names no longer share an empty key, and folds NFKC + full case folding, so the two Unicode spellings of an accented name are one page identity and `ß` / Greek final sigma fold like their peers.
  - *Release note:* Discarded names no longer come back through synth, and a slash in a candidate name no longer hides the stub (#282).
- **Breaking:** **`llmwiki add` / MCP `wiki_add` default is raw + site rebuild; synthesize is opt-in (#273)** — bare `add` / `wiki_add` writes under `raw/docs/` and rebuilds the site; it no longer runs synthesis unless you pass `--synthesize` / `synthesize: true`. `--no-synthesize` is a warn+no-op alias for one release; `--no-build` / `no_build` still skips the site rebuild. Stdin `llmwiki add -` and MCP `content` use piped-text conversion (`source: "piped"`, no tempfile provenance). MCP `wiki_add` is a thin proxy onto shared `run_add`. Scripts that relied on synth-on-add must add `--synthesize` (or run `llmwiki synth` afterward).
  - *Release note:* `add` / `wiki_add` land raw and rebuild the site by default; pass `--synthesize` / `synthesize: true` for wiki source pages (#273).
- **Synth vocabulary carries topic kind (#257)** — each Existing topics row may include `kind="entity|concept"` when known (known-names cache, else unique page/pending filing); the source-page prompt requires Connections parentheses to be exactly `entity` or `concept` (no free-form type nouns) and to copy a listed topic's kind. Docs contrast job-1 known-names preparation vs offline candidates harvest / `--candidates-only`. Label-only backlog still uses `llmwiki migrate topic-kinds` (#174) — do not force a full re-synth for kinds alone.
  - *Release note:* Source synth sees known topic kinds in vocabulary; free-form Connections kinds forbidden; use `migrate topic-kinds` for label-only catch-up (#257).
- **Breaking:** **Synth stops cleanly on Ctrl+C or a backend usage limit (#181)** — every backend (Claude CLI, Cursor Agent CLI, Ollama) raises the new `BackendUsageLimitError` (with the provider's reset time) when its error message carries account-quota wording (`hit your … limit`, `usage limit`, `quota exceeded`, …), matched by the shared `usage_limit_from_text`; a bare HTTP 429 or rate-limit throttling stays a per-source error. The first such result — including one from the known-names preparation, which then defers the whole queue without a page call — or Ctrl+C cancels the queue once; pages in flight finish through the normal per-source handling (state entry, pending removal, counts) and the run keeps the success-path bookkeeping (single `wiki/log.md` entry marked stopped early with `Deferred:`, state save, pending refresh, index rebuild, usage). Summary gains `deferred` and `usage_limit`. `synth` harvests what landed and exits `75` (usage limit) or `130` (Ctrl+C); `all` propagates both while still running later stages, and a `--lint-fail` failure now merges into the first non-zero code instead of returning `2` over it. Claude and Cursor CLI children start in their own session so a terminal Ctrl+C no longer kills in-flight pages; a second Ctrl+C while they drain kills those child processes (process group SIGTERM, then SIGKILL) and ends the run without recording the killed pages — Ollama has no child process and waits for in-flight requests up to its timeout. The known-names preparation now prints a progress line (candidate count, prompt size) before its language-model call. The `install-automation` wrapper now logs and exits with the real exit code.
  - *Release note:* A usage-limit or Ctrl+C stop finishes in-flight pages, defers the rest, and exits `75` / `130`; `all` no longer hides these (or an earlier failure) behind `0` / `2`; reinstall automation for a truthful `EXIT:` line (#181).
...
- **`migrate discarded-topic-links` converges instead of looping at exit 1 forever (#282)** — a recorded merge whose name has zero links anywhere in the wiki is no longer treated as a pending suggestion: it is left out of `report["recorded_merges"]` and the printed report, and no longer flips the exit code on its own. Previously every recorded merge the reason file knew about counted toward `merges_skipped`, even one with nothing left to rewrite, so a vault could get permanently stuck reporting `pages changed: 0` / `links unlinked: 0` and exiting 1 on every run, including after every suggested `--redirect` had already been applied. `--force` behavior is unchanged.
  - *Release note:* `migrate discarded-topic-links` now exits 0 once every actionable recorded merge has been redirected or forced, instead of getting stuck on merges that never had any links (#282).
- **Source pages filed under a stale name no longer stay pending forever (#265)** — new offline `llmwiki migrate source-page-paths` moves each real `wiki/sources/` page whose filename differs from the one synth derives for its `source_file:` to that derived path (doc `--part-NN` sets move together), rewrites `[[old-stem]]` links and `sources:` entries across `wiki/` (a bare stem several pages share follows the one candidate that links back to the referring page; otherwise it is reported as ambiguous and left alone), updates a converter-shaped `title`, and records synth state so `synth`, `--estimate` and Home treat the source as done. Collisions at the derived path are reported and left untouched; `--dry-run` writes nothing; a second run is a no-op. `synth` now prints one `skipped N source(s) already claimed by a real page under another name` line with the migrate hint instead of one line per source.
  - *Release note:* Run `llmwiki migrate source-page-paths --vault <vault>` once if `synth` keeps skipping sources that already have a real page (#265).
- **Number-shaped session slugs keep their text in the page name (#265)** — the frontmatter reader now keeps `slug:` as written text (`llmwiki._frontmatter.TEXT_KEYS`), so a raw `slug:` such as `0123`, `68657849` or `12e4` yields `<date>-<slug>` with the exact written text instead of falling back to the raw filename and doubling the date. Existing doubled-date pages are moved by `migrate source-page-paths`.
  - *Release note:* Sessions with number-like slugs get `<date>-<slug>` page names; run `migrate source-page-paths` to rename old ones (#265).
...
- **Claude Code control tags no longer leak into session `description:` or Conversation (#229)** — convert strips / collapses `<local-command-caveat>`, `<command-name>`, `<command-message>`, `<command-args>`, `<local-command-stdout>` (and similar) envelopes, plus `[SYSTEM NOTIFICATION …]` banners and `<task-notification>` blocks from background-task events. Caveat-only / notification-only turns are omitted; slash-command turns become `/name` or `/name <args>` when `<command-args>` is non-empty; user-prompt newlines are preserved as markdown hard breaks so multi-line turns render as multiple lines. Subtitle selection (including whether an injected dump’s first line can win) is governed by #249 scoring / assigned names — not a separate dump-skip list on the description path. Already-synced vaults need `llmwiki sync --force` (then `build`) so `raw/sessions/` is re-converted.
  - *Release note:* Claude local-command / slash-command / system-notification envelopes are cleaned at convert time (args kept; newlines preserved); re-sync with `--force` to refresh existing sessions (#229).
- **Session “On this page” TOC no longer overlaps the hero (#229)** — session detail pages mount the TOC in a Raw-style sticky `.doctree-layout` column below the hero (left of the article), not as a body-fixed panel at `top: 88px`. The nav appears when the article has ≥2 headings and uses the same `max-width: 860px` collapse as Raw (visible on normal desktop widths, not only ultra-wide). Session `description:` (#471) is rendered **inside** the hero `.container` with the title/meta chips (not as a sibling after `</section>` with negative-margin CSS). Selection quality of that subtitle is tracked in #246. Rebuild the site to pick up the layout.
  - *Release note:* Session TOC sits sticky under the hero like the Raw documents sidebar; shows from ≥2 headings at normal desktop widths (#229).
...
- **Release checklist defaults to demo content refresh (#225)** — `RELEASE_PROCESS.md` and the `/release` skill refresh `demo/` sessions (`generate_demo_sessions.py` with a release-day `--today`) and docs (`refresh_demo.py`) before tagging unless the human **explicitly** opts out; Pages version assert alone is not a content refresh. Prefer re-synthing changed session sources + docs-only, and wait for the human when they will run synth. Script docstrings and `REFRESH_DEMO.md` cross-link the same path.
  - *Release note:* Before tagging, regenerate the demo corpus by default; skip only when told to; CI still never invents demo sessions (#225).
...
- **Analytics Tokens avg hid its divisor (#223)** — the per-session average still divides by sessions with ≥1 non-zero token category (not the hero session count), but the sub-label now reads e.g. `10.0K / session (4 with token data)`, and a one-line note under the tile clarifies that totals are cumulative billed throughput (including cache_read), not context-window occupancy.
  - *Release note:* Analytics Tokens avg labels the token-data session count and notes billed throughput vs context occupancy (#223).
- **README claimed CI runs Python 3.12 and 3.13** — leftover from before the matrix was narrowed to 3.12 only. README now matches `ci.yml` / CONTRIBUTING (verify 3.12; newer interpreters installable but not matrix-tested).
  - *Release note:* Docs no longer advertise a 3.13 CI job that does not exist.
- **The demo site never republished, so it served a four-release-old build (#213)** — `pages.yml` was `workflow_dispatch`-only (#69), which meant the live site only moved when someone remembered to press a button; it had been showing v1.5.0 since v2.1.0 shipped. It now also deploys on every `v*.*.*` tag, alongside `release.yml`. Deploy on push to `main` stays off — the demo tracks releases, not merges. The deploy job reads `manifest.json` back off the live URL via `scripts/check_live_version.py` and fails when the served version isn't the `__version__` of the commit it just published, so a green run means the site really serves that release rather than merely that GitHub accepted the artifact. Both triggers assert against the checked-out `__version__`, so a manual dispatch is checked as strictly as a tag, and a tag pushed without the version bump fails instead of publishing a mislabelled site. No separate weekly freshness workflow — release/tag deploys are the gate.
  - *Release note:* Pushing a version tag now republishes the demo site and verifies the live version, instead of leaving it to a manual run (#213).
...
- **`install-agent-kit` prunes retired commands (#214)** — the install now removes `/wiki-*` command and skill files it previously wrote that the kit no longer ships, so a destination set up before a removal stops offering broken commands (`/wiki-export-marp`, `/wiki-synthesize`). Deletion is gated on content, not on the name: a file goes only while it still hashes to a revision llmwiki shipped at that path. Those digests come from a packaged retired-path list (every revision of each retired file) plus a new `<dest>/.llmwiki-agent-kit.json` manifest recording the version and a `path → sha256` of what was installed. A file the command never installed is never touched, and neither is a retired command you edited — an unrecognised digest leaves it in place and reports it as `kept`. Because only bytes llmwiki wrote are ever deleted, the prune makes no backup. Manifest entries that escape the destination are ignored, an older list-shape manifest authorises no deletion, and `--dry-run` reports the prune without deleting anything.
  - *Release note:* Re-run `llmwiki install-agent-kit --dest PATH` to clear retired `/wiki-*` commands from an older install; only untouched copies of files llmwiki itself wrote are removed, so anything you customised stays put. The new `<dest>/.llmwiki-agent-kit.json` manifest is safe and useful to commit alongside `.claude/commands/` — it is what lets the next upgrade recognise its own files (#214).
...
Theme: Cursor IDE ingest + sync lookback, lifecycle CLI help / migrate wrapper, MCP six-tool surface.
...
- **Demo vault case-collision cleanup + repo hygiene tests (#160)** — removed `demo/wiki/candidates/entities/LLMWiki.md`, a stray case-duplicate of the dominant-spelling `demo/wiki/candidates/entities/llmwiki.md`; the two coexisted fine on case-sensitive Linux but collided on case-insensitive filesystems (macOS default, Windows). Added `tests/test_case_insensitive_paths.py` (case-folded `git ls-files` collisions) and moved the privacy username guard from a standalone `ci.yml` shell step into `tests/test_privacy_username.py`, so both checks run under the standard `pytest tests/` suite. The harvest writer's exact-spelling wikilink behavior is unchanged; that's tracked in #204.
  - *Release note:* No user-facing behavior change; the demo vault no longer ships a case-duplicate page, and pytest catches case collisions and the privacy username leak before merge (#160).
- **Docs link-check hygiene (#107)** — repair relative links under `docs/modes/agent/`, drop dead external URLs (`claude-code` session-history, deleted `Ss1024sS/LLM-wiki` research entry), point `synthesis-cost.md` at repo-root `model_pricing.csv`, exclude `{{__llmwiki_*}}` build-time placeholders from lychee (raw and URL-encoded), point demo-adopter docs at committed `demo/wiki/entities/` pages, dedupe tracking issues via `last-issue-action` + `issue-number` (replacing invalid `update_existing`), and fire link-check on `main` (not `master`) so regressions surface on push.
  - *Release note:* Weekly link-check stops re-opening tracking issues once this ships; stale duplicate tracking issues can be closed.
...
- **MCP `wiki_lint` returns `llmwiki lint --json`** — `orphans` / `broken_links` keys replaced by `summary`, `issues`, `total_pages`, `disabled_rules`, `ran` (#150).
...
- **Lint rule `entity_consistency` removed** — `entity_type` frontmatter is inert metadata (#102).
...
- **Page-kind reference** — `docs/reference/page-kinds.md` (#109).
...
- **`llmwiki export` and `llmwiki reindex` CLI** — `build` and automatic reconciliation replace them (#82).
