...
- **Home / Analytics candidates review gate (#84)** — Home pipeline adds a **To review** column (pending `wiki/candidates/` count on the Total row) plus a Candidates collapse and Commands rows for `synthesize --candidates-only`, `candidates list` / promote, and vault-cwd launchers for Claude Code / Codex / Cursor / Gemini. Analytics shows **Candidates to review** (pending + stale). `llmwiki build` recounts candidates into `synth.pipeline` on every build so the gate cannot silently disappear after promote/discard.
