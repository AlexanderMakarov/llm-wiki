...
- **Claude Code control tags no longer leak into session `description:` or Conversation (#229)** — convert strips / collapses `<local-command-caveat>`, `<command-name>`, `<command-message>`, `<command-args>`, `<local-command-stdout>` (and similar) envelopes, plus `[SYSTEM NOTIFICATION …]` banners and `<task-notification>` blocks from background-task events. Caveat-only / notification-only turns are omitted; slash-command turns become `/name` or `/name <args>` when `<command-args>` is non-empty (Conversation only — `description:` skips slash invocations and prefers the first real user prose); injected command/skill markdown dumps (`# Title` + `## Arguments`, `@path.md` + `ARGUMENTS:`) are skipped for `description:`; user-prompt newlines are preserved as markdown hard breaks so multi-line turns render as multiple lines. Already-synced vaults need `llmwiki sync --force` (then `build`) so `raw/sessions/` is re-converted.
  - *Release note:* Claude local-command / slash-command / system-notification envelopes are cleaned at convert time (args kept; dumps skipped; newlines preserved); re-sync with `--force` to refresh existing sessions (#229).
