...
- **BREAKING — the MCP `wiki_lint` tool returns the `llmwiki lint --json` report (#150)** — keys `orphans`, `orphan_count`, `broken_links`, and `broken_link_count` are replaced by `summary`, `issues`, `total_pages`, `disabled_rules`, and `ran`. Optional arguments: `rules`, `min_refs`. See [`docs/UPGRADING.md`](docs/UPGRADING.md).
- **`lint` honours the candidate-harvest significance threshold, and gains `--min-refs N` (#150)** — `link_integrity` skips targets named by fewer than **N** distinct source pages (default **3**, shared with harvest via `llmwiki.vault_settings.DEFAULT_MIN_REFS`). `llmwiki lint --min-refs N` and `llmwiki all --min-refs N` thread the value through lint and harvest.
...
- **A failing `lint` now records that it ran (#150)** — the exit code is computed after the `last_lint_run_at` state update.
- **`lint` reports findings in deterministic order (#150)** — `run_lint` sorts each rule's issues by `(page, message)` so CLI and MCP payloads match byte-for-byte.
...
- **Per-vault lint opt-outs in `<vault>/llmwiki.json` (#150)** — `lint.disabled_rules` as a list or `{rule: reason}`; skipped rules are named in every report; unknown names exit 2.
- **`lint --fail-on-warnings` (#150)** — warning-severity findings fail the gate; disabled rules never stop it.
