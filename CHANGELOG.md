...
- **`llmwiki add` extracted nothing from real article pages (#42 regression)** — the `favor_precision=True` added for boilerplate removal was too aggressive: on real pages (armgate.am) trafilatura discarded the entire article as low-confidence and returned only a skip-to-content link (~33 chars). Dropped it — trafilatura's default recall already strips site chrome while keeping the article body (verified end-to-end: an armgate doc page goes from 33 chars of nav to 1870 chars of clean content, no navigation). The kwargs test now asserts `favor_precision` is unset, so the regression can't return.
