...
- **Number-shaped session slugs keep their text in the page name (#265)** — a raw `slug:` such as `0123`, `68657849` or `12e4` (parsed as a number by the frontmatter reader) now yields `<date>-<slug>` with the exact written text instead of falling back to the raw filename and doubling the date. Existing doubled-date pages are moved by `migrate source-page-paths`.
